...
- **Set the viewport to the real screen at setup.** Playwright emulates a fixed viewport (seen: 1440×812 on a 900px screen), so `requestFullscreen` succeeds yet the canvas is letterboxed. Call `browser_resize` with `screen.width × screen.height` (read them via `browser_evaluate`) before the first tick. Permanent fix: `~/mcp/playwright-mcp.config.json` with `contextOptions.viewport: null` and `launchOptions.args: ["--start-fullscreen"]` — takes effect on MCP restart. There is no `--headed` flag (headed is the default; passing it kills the server with "unknown option").
- **Screenshot only AFTER `browser_evaluate` confirms the new source is live.** A screenshot fired in the same tool block as an edit races HMR and shows the OLD frame — this misled three consecutive decisions in one set. Confirm with `window.cranes.shader.includes('<unique new token>')`, then shoot.
- **Measure a spread, not a frame.** If a screenshot and a meter taken seconds apart disagree, the frame is oscillating — that IS the "flashy" the user is complaining about. Sample the canvas 8× over ~3.5 s in one `browser_evaluate` and report `lumSpread`; a stable look is < 0.1.
- **When `lumMin` won't move under palette edits, grep every `col +=` after `mix(bg, col, alpha)`.** An unmasked additive term anywhere floors the whole frame and beats any amount of palette tuning (see `shaders/redaphid/lattice-interactive/HANDOFF.md` §0.3).
