...
## Todo
- [ ] iter19: R&D output is `.claude/vj-candidates/patches.json` + `verify.py`, not the QUEUE.md the brief asked for, and the json parsed to 0 items under the keys tried. Tell the R&D agent the exact schema you read (or read its top-level keys once) before the next tick, or the team's output is wasted.
...
## History of changes
- iter19: hue rides the arc — depth hue-span + gArc*0.60, flux twinkle gArc*0.10 into hue (colour lane only). Pre-move spread 0.042 at arc 0.04 confirms the spread was intensity-side.
