...
- [ ] iter21: hueMix.red 0.23 on a bass-heavy passage (was 0.09 on treble). Red = lines in the selector's transition band; its share is passage-dependent, so judge it across two passages, not one. If red > 0.20 persists next tick → selector 0.44–0.56 → 0.46–0.54 (chroma already cut on the orange side; the angle stays).
