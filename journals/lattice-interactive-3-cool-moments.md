...
## Status
**Iter 21 (21:44).** Proactive, subtractive, colour lane: removed the iter-19 whole-frame flux twinkle (`+ gArc*0.10*clamp(spectralFluxZScore,0,1)` on the hue coordinate). Reason: under the near-binary orange/purple selector `smoothstep(0.44,0.56)` a whole-frame hue shift on every transient flips threshold-adjacent lines between the two colours — a colour STROBE that scales in with gArc; R&D flagged it (C8); user has said "less flashy" / no shivering. Numbers pending.
...
## History of changes
- iter21: removed flux twinkle on the hue coordinate (R&D C8) — under the binary palette selector it became a two-colour strobe on every transient. Subtractive.
