...
- [ ] iter22: C4 rush terms were measured on a quiet-bass window (bass spring ≤0.30, kickExcess ~0) — re-check lumSpread on the next kick-heavy passage; if it exceeds ~0.10 the 0.008 flux term goes first (fires on any transient).
- [x] 21:57 check at arc 0.765: whiteish 0.0000, clip 0, red 0.08, sat 0.779 — passed, no fallback (ceiling stays 0.90).
- [ ] ~22:05, arc 1.0: take a full 8-frame set (lum, dark, black, clip, whiteish, sat, lumSpread, hueMix, lumMax vs the 0.90 cap) as the 'hypercolor landed' record. This is the endpoint of the user's one-hour build.
...
## History of changes
- iter23: arc 0.765 whiteish check passed (0.0000). R&D C7 line weight ← energy (Iris LINE_THICK; thins in quiet, never above current width). Width lane.
