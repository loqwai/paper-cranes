...
- [ ] iter20c: the iter18 Todo about `gArc` wall-clock reset just happened, 6 min after it was written. Team agents must NOT create `.frag` files anywhere under the project root during a set (Vite watcher → full reload); `.frag.txt` outside the root. Verify the R&D agent complied before its next write.
...
- [x] iter18 whiteish check at arc 0.528: whiteish 0.0000, spread 0.033 — passed, but only because the drive never reached the cap. 20b raised the drive; RE-CHECK whiteish + hueMix.other on the 20b numbers and again at arc≈0.75 (~21:45). Ceiling is now 0.90; fallback 0.85 if whiteish > 0.01.
- [x] iter20b: `hueMix.other` 0.00 — palette lock is airtight, no colour path bypasses lush().
- [ ] iter20c: confirm hueMix after the 82°/chroma×0.60 fix — red should drop toward 0 and orange take its share; if red persists > 0.10, lower orange-side chroma further (0.60→0.50) before touching the hue angle again.
...
## History of changes
- iter20c INCIDENT: full page reload (R&D agent created new .frag files under the project root → Vite watcher). gArc→0, navZoom→1.0; fullscreen held. Recovered via `&navZoom=0.263` URL + T0 re-anchor to arc≈0.6. New rule: no new .frag anywhere under the root mid-set.
