...
async function setupAnalyzer() {
  ...
  const {default: analyzer} = await import(/* @vite-ignore */ `https://esm.sh/hypnosound@1.14.0/src/audio/${analyzerName}.js`)

  if (!analyzer) throw new Error(`Analyzer ${analyzerName} not found`)

  const { makeCalculateStats } = await import('https://esm.sh/hypnosound@1.14.0/src/utils/calculateStats.js')
