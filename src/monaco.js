    ...
    monaco.languages.registerHoverProvider('glsl', {
        provideHover: (model, position) => {
            ...
            const audioFeatureDescriptions = {
                ...
                beat: 'True when a beat is detected. Perfect for synchronizing visual changes with the rhythm. When it works. But it usually doesn\'t. Consider onset/timeSinceOnset/onsetEnvelope instead.',
                onset: 'True for exactly one frame when the onset detector fires (adaptive spectral flux threshold with a refractory period). Edge-triggered, unlike beat. Tune live via onset_sensitivity, onset_refractory_ms, onset_low_hz/onset_high_hz.',
                timeSinceOnset: 'Seconds since the last detected onset (starts at 1000 before the first one). The primitive for designed responses: shape it with onsetEnvelope or your own curve, e.g. exp(-timeSinceOnset * 4.0).',
                onsetStrength: 'How hard the last onset hit, 0 to 1 (flux excess over the threshold). Latched until the next onset — multiply your envelope by it so big hits respond bigger.',
                onsetFlux: 'Raw per-bin spectral flux feeding the onset detector. Mainly for diagnostic shaders.',
                onsetThreshold: 'Current adaptive threshold of the onset detector. Mainly for diagnostic shaders — flux crossing it (outside the refractory period) fires an onset.',
                onsetEnvelope: 'onsetEnvelope(attack, release): one-shot envelope retriggered by each onset — rises 0 to 1 over attack seconds, then decays exponentially with the release time constant. Unlike the animate* oscillators, it fires once per event. Example: float kick = onsetEnvelope(0.01, 0.25) * (0.5 + onsetStrength);',
            ...
            if (description) {
                return {
                    contents: [
                        ...
                        { value: !word.word.startsWith('animate') ? `Available variables: \n- ${word.word}Normalized (0-1)\n- ${word.word}Mean\n- ${word.word}Median\n- ${word.word}StandardDeviation\n- ${word.word}ZScore\n- ${word.word}Min\n- ${word.word}Max\n- ${word.word}Slope (linear regression trend)\n- ${word.word}Intercept (regression baseline)\n- ${word.word}RSquared (trend confidence 0-1)` : '' }
                    ]
                }
            }
        }
