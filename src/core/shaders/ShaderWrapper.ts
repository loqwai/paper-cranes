...
import type { ShaderTransformer, AudioUniforms, KnobUniforms } from '../../types/shaders.js'

const PAPER_CRANES_MARKER = '31CF3F64-9176-4686-9E52-E3CFEC21FE72'
const SHADER_VERSION = '#version 300 es'
const PAPER_CRANES_DEFINE = '#define PAPER_CRANES 1'

const PRECISION_DECLARATION = 'precision highp float;'
const OUTPUT_DECLARATION = 'out vec4 fragColor;'

export class ShaderWrapper {
  static transform: ShaderTransformer = (shader: string): string => {
    if (!shader || typeof shader !== 'string') {
      throw new ShaderError('Invalid shader input: must be a non-empty string')
    }

    const trimmedShader = shader.trim()
    if (!trimmedShader) {
      throw new ShaderError('Invalid shader input: empty shader content')
    }

    const lines = trimmedShader.split('\n')
    const firstLine = lines[0]!

    // Handle shaders that already have version directive
    ...
    // Handle Shadertoy-style shaders with mainImage function
    if (trimmedShader.includes('mainImage')) {
      return this.createMainImageShader(trimmedShader)
    }

    // If no mainImage function, this is not a valid shader
    throw new ShaderError(
      'Shader does not contain mainImage function. It should look like this: void mainImage( out vec4 fragColor, in vec2 fragCoord ) { ... }'
    )
  }

  private static createMainImageShader(shader: string): string {
    try {
      const shaderToyUniforms = this.getShaderToyCompatibilityUniforms()
      const audioUniforms = this.getAudioUniforms()
      const knobUniforms = this.getKnobUniforms(shader)
      const paperCranesUtilities = this.getPaperCranesUtilities()
      ...
      return [
        ...
        knobUniforms,
        '',
        paperCranesUtilities,
        this.getFrameBufferFunctions(),
        `// ${PAPER_CRANES_MARKER}`,
        shader,
        '',
        'void main(void){',
      ...
      throw new ShaderError('Failed to create mainImage shader wrapper', undefined, err)
    }
  }

  private static getKnobUniforms(shader: string): string {
    try {
      const existingKnobs = new Set(
        [...shader.matchAll(/uniform\s+float\s+knob_(\d+)/g)].map(m => parseInt(m[1]!, 10))
      )

      const knobDeclarations = Array.from({ length: 200 }, (_, i) => i + 1)
        .filter(i => !existingKnobs.has(i))
        .map(i => `uniform float knob_${i};`)

      return knobDeclarations.join('\n')
    } catch (err) {
      throw new ShaderError('Failed to generate knob uniforms', undefined, err)
    }
  }

  private static getShaderToyCompatibilityUniforms(): string {
    return [
      'uniform vec4 iMouse;',
      'uniform float iTime;',
  ...
  private static getAudioUniforms(): string {
    try {
      const audioFeatures = getFlatAudioFeatures()
      const featureKeys = [...Object.keys(audioFeatures), 'beat']

      return featureKeys
        .sort()
        .map(feature => `uniform ${feature === 'beat' ? 'bool' : 'float'} ${feature};`)
        .join('\n')
    } catch (err) {
      throw new ShaderError('Failed to generate audio uniforms', undefined, err)
    }
  }

  private static getFrameBufferFunctions(): string {
    return [
  ...
  private static getPaperCranesUtilities(): string {
    return `
uniform float time;
