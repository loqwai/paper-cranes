...
vec3 calcNormal(in vec3 pos) {
  ...
  vec3 nor = vec3(map(pos + eps.xyy).dist - map(pos - eps.xyy).dist, map(pos + eps.yxy).dist - map(pos - eps.yxy).dist, map(pos + eps.yyx).dist - map(pos - eps.yyx).dist);
...
Surface march(in Camera cam) {
  ...
  for(int i = 0; i < NUM_OF_TRACE_STEPS; i++) {
    if(abs(h) < INTERSECTION_PRECISION || d > MAX_TRACE_DISTANCE)
      break;
    p = cam.ro + cam.rd * d;
    Model m = map(p);
    h = m.dist;
...
vec3 render(Surface surface, Camera cam, vec2 uv) {
  ...
  if(surface.id > -1.) {
    ...
    float t = time;
    vec3 col = mix(mix(vec3(.8, .3, .6), vec3(.6, .3, .8), cos(length(pos.xy) * 3. + t) * cos(length(pos.yz + 5.) * 1. + t) +
      sin(length(pos.zx) * 10. + 10. + t) * .2 * cos(length(pos.zy))), vec3(1), smoothstep(0., .1, cos(surface.colour.r * 40.)));
