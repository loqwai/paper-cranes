...
float box_set(vec3 pos) {
    vec3 pos_origin = pos;
    float dynamicMovement = sin(time * 0.4 + spectralCentroidNormalized) * 2.5;

    // Apply audio feature modifications
    pos = pos_origin;
    pos.y += dynamicMovement;
    pos.xy *= rot(0.8 + spectralCentroidZScore * 0.2); // Rotate based on spectralCentroidZScore
    float box1 = box(pos, 2.0 - abs(sin(time * 0.4)) * 1.5 + energyNormalized);

    pos = pos_origin;
    pos.y -= dynamicMovement;
    pos.xy *= rot(0.8 + spectralCentroidZScore * 0.2);
    float box2 = box(pos, 2.0 - abs(sin(time * 0.4)) * 1.5 + energyNormalized);

    pos = pos_origin;
    pos.x += dynamicMovement;
    pos.xy *= rot(0.8 + spectralCentroidZScore * 0.2);
    float box3 = box(pos, 2.0 - abs(sin(time * 0.4)) * 1.5 + energyNormalized);

    pos = pos_origin;
    pos.x -= dynamicMovement;
    pos.xy *= rot(0.8 + spectralCentroidZScore * 0.2);
    float box4 = box(pos, 2.0 - abs(sin(time * 0.4)) * 1.5 + energyNormalized);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(col.b < 0.1 && col.r < 0.1 && col.g < 0.1) {
        discard;
    }
    // rotate the color via hsl when the energy is high
    col = hsl2rgb(vec3(getGrayPercent(vec4(col, 1.0)), 1.0, 0.5));
