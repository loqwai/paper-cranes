...
// @name: totality alive
//https://visuals.beadfamous.com/?shader=claude/wip/eclipse/2&controller=wavelet-ease&wavelet=true&fullscreen=true&name=Totality
//
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float R  = 0.082;                          // the moon — SMALL now, and alive inside.
    float r  = length(uv);
    ...
    float rayAmp  = 0.62 + 0.62 * bass + 0.38 * lift + 0.45 * band;
    ...
    col *= smoothstep(R - aa * 1.5, R + aa * 0.5, r) * 0.88 + 0.12;
    ...
    float ember = smoothstep(0.28, 0.70, en) * pow(ir, 1.7);
    vec3  emberCol = mix(vec3(0.90, 0.10, 0.03), vec3(1.0, 0.48, 0.12), ember);
    float inside = 1.0 - outside;
    col += emberCol * ember * inside * (0.50 + 1.2 * bass + 1.4 * kick);
    col += vec3(1.0, 0.55, 0.22) * pow(ir, 7.0) * inside * (0.35 + 0.8 * lift);
