...
// beat/drop detection, and the best INDEPENDENT animation features for the current audio
// (measured live, all pairwise |corr| < 0.45). The point: these lines are smooth, lively,
// independent, and locked to the music — ready to drive real visuals.
//
// Run with: ?shader=claude/wip/wavelet-scope/proof&wavelet=true&controller=wavelet-ease
//
...
//
// Smooth lines = spring-eased (best easing found). Trigger = raw (sharp on purpose).
...
// beat/drop trigger lane: clean flash bar when bassHit fires (a detected beat/drop)
