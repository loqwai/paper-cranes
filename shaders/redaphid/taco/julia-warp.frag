...
// http://localhost:6969/?shader=redaphid/taco/julia-warp&image=images/taco-stencil.png&controller=taco-kandi
// Tab-audio mode (Spotify/SoundCloud — Chrome desktop):
// http://localhost:6969/?shader=redaphid/taco/julia-warp&image=images/taco-stencil.png&controller=taco-kandi&audio=tab
// Live jam editor:
// http://localhost:6969/jam.html?shader=redaphid/taco/julia-warp&image=images/taco-stencil.png&controller=taco-kandi&audio=tab
// Seeded variants (each phone gets a unique look):
// http://localhost:6969/?shader=redaphid/taco/julia-warp&image=images/taco-stencil.png&controller=taco-kandi&seed=0.618&seed2=0.755&seed3=0.892&seed4=0.029
// http://localhost:6969/?shader=redaphid/taco/julia-warp&image=images/taco-stencil.png&controller=taco-kandi&seed=0.236&seed2=0.373&seed3=0.51&seed4=0.647
// http://localhost:6969/?shader=redaphid/taco/julia-warp&image=images/taco-stencil.png&controller=taco-kandi&seed=0.854&seed2=0.991&seed3=0.128&seed4=0.265
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (hasHistory) {
        // Per-frame inward pull: small, keeps echo fluid.
        ...
        // BANDING FIX (iter 28): jitter the sample point so adjacent ring generations
        // overlap. Without jitter each frame produces a sharp concentric ring step
        // visible as a hard band; small per-pixel offset blurs the ring transitions.
        ...
        // Smooth decay (no per-frame hue rotation — that was the second banding source).
        float echo_decay = 0.90 + knob_16 * 0.06;
        echo_prev *= echo_decay;
        // BANDING FIX: switched from max() to additive-with-cap blend. max() preserves
        // brightest at each ring → hard rings. Continuous mix smooths the transitions.
        float echo_strength = (1.0 - silhouette) * (0.4 + knob_16 * 0.4);
        col += echo_prev * echo_strength * 0.6;
    }
    ...
    if (knob_14 > 0.01 && hasHistory) {
        ...
        julia_prev *= 0.86;
        // BANDING FIX (iter 29): removed per-frame oklch hue rotation toward CORONA_HUE
        // (was painting visible color bands per ring generation, same issue as logo echo
        // before iter-28 fix). Julia warp now preserves whatever hue the previous frame had.
        // Switched from max() blend to additive (avoids hard-ring concentric steps).
        float julia_strength = (1.0 - silhouette) * knob_14 * 0.5;
        col += julia_prev * julia_strength;
    }
