...
// http://localhost:6969/?shader=redaphid/taco/radiation&image=images/taco.png&controller=taco-kandi
// Tab-audio mode (Spotify/SoundCloud — Chrome desktop):
// http://localhost:6969/?shader=redaphid/taco/radiation&image=images/taco.png&controller=taco-kandi&audio=tab
// Live jam editor:
// http://localhost:6969/jam.html?shader=redaphid/taco/radiation&image=images/taco.png&controller=taco-kandi&audio=tab
// Seeded variants (each phone gets a unique look):
// http://localhost:6969/?shader=redaphid/taco/radiation&image=images/taco.png&controller=taco-kandi&seed=0.618&seed2=0.755&seed3=0.892&seed4=0.029
// http://localhost:6969/?shader=redaphid/taco/radiation&image=images/taco.png&controller=taco-kandi&seed=0.236&seed2=0.373&seed3=0.51&seed4=0.647
// http://localhost:6969/?shader=redaphid/taco/radiation&image=images/taco.png&controller=taco-kandi&seed=0.854&seed2=0.991&seed3=0.128&seed4=0.265
...
vec4 getTacoRegions(vec2 uv) {
    ...
    float ink = tex.a * step(maxRGB, 0.5);
    float isShell   = tex.a * step(0.7, tex.r) * step(tex.g, 0.7);
    float isFilling = tex.a * step(0.7, tex.g) * step(tex.r, 0.7);
...
vec3 shellFractal(vec2 fragUV) {
    ...
    // Tight zoom so we see fine fractal detail within the shell.
    vec2 z = (fragUV - 0.5) * 3.5;
    z.x *= iResolution.x / iResolution.y;
    // Per-device Julia constant — fract(seed)/fract(seed2) pick a fractal
    // family on the unit circle ±0.85. Music adds tiny modulation (no phase
    // discontinuity since we lerp through fract).
    float jt = iTime * 0.05 + bass_smooth * 0.3;
    float jr = mix(0.55, 0.85, fract(seed));
    vec2  jc = vec2(
        cos(jt + fract(seed) * TAU) * jr + (spectralRoughnessNormalized - 0.5) * 0.02,
        sin(jt + fract(seed2) * TAU) * jr + (spectralCentroidNormalized - 0.5) * 0.02
    );
    ...
    // Map iter+traps to a plasma-family palette (NOT wooli's icy blue —
    // we want warm orange-amber for the taco shell to read as a tortilla).
    // Three oklch anchors in the orange-amber-cream family. Iter 52: high-pitch
    // notes lift cream toward gold (pitch=0 → cream, pitch=1 → gold). Bounded
    // ±0.06 rad so always stays in plasma family. Independent from radiation,
    // photon ring, etc. — only the SHELL responds to pitch.
    float pitch_lift = (pitchClassNormalized - 0.5) * 0.12;  // ±0.06 rad
    vec3 deep   = oklch2rgb(vec3(0.30, 0.10, CORE_HUE - 0.05));        // dark amber
    vec3 mid    = oklch2rgb(vec3(0.55, 0.15, CORE_HUE));               // hot orange
    vec3 bright = oklch2rgb(vec3(0.78, 0.13, CORE_HUE + 0.10 + pitch_lift));  // cream→gold on melody
    // Iter as primary mixer; traps add detail bands.
    float t1 = clamp(sIter / 8.0, 0.0, 1.0);
    vec3 col = oklabmix(deep, mid, smoothstep(0.0, 0.5, t1));
    col = oklabmix(col, bright, smoothstep(0.0, 0.20, sqrt(trapY)));
    // Subtle music breathing — bass_smooth lifts brightness slightly
