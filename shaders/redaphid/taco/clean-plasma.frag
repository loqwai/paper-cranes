...
// http://localhost:6969/?shader=redaphid/taco/clean-plasma&image=images/taco-stencil.png&controller=taco-kandi
// Tab-audio mode (Spotify/SoundCloud — Chrome desktop):
// http://localhost:6969/?shader=redaphid/taco/clean-plasma&image=images/taco-stencil.png&controller=taco-kandi&audio=tab
// Live jam editor:
// http://localhost:6969/jam.html?shader=redaphid/taco/clean-plasma&image=images/taco-stencil.png&controller=taco-kandi&audio=tab
// Seeded variants (each phone gets a unique look):
// http://localhost:6969/?shader=redaphid/taco/clean-plasma&image=images/taco-stencil.png&controller=taco-kandi&seed=0.618&seed2=0.755&seed3=0.892&seed4=0.029
// http://localhost:6969/?shader=redaphid/taco/clean-plasma&image=images/taco-stencil.png&controller=taco-kandi&seed=0.236&seed2=0.373&seed3=0.51&seed4=0.647
// http://localhost:6969/?shader=redaphid/taco/clean-plasma&image=images/taco-stencil.png&controller=taco-kandi&seed=0.854&seed2=0.991&seed3=0.128&seed4=0.265
...
float getOutlineDistance(vec2 uv) {
    ...
    float sCenter = getTacoMask(uv).x;
    ...
    for (int ri = 0; ri < N_RADII; ri++) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            float sN = getTacoMask(uv + vec2(cos(a), sin(a)) * r).x;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // rotation. Pure positional jitter. Brightness/color stay anchored.
    {
        ...
        if (shiver_gate > 0.02) {
            ...
            float shivered_ink = getTacoMask(shivered_uv).y;
