...
// http://localhost:6969/?shader=redaphid/taco/direct-kick&image=images/taco.png&controller=taco-kandi
// Tab-audio mode (Spotify/SoundCloud — Chrome desktop):
// http://localhost:6969/?shader=redaphid/taco/direct-kick&image=images/taco.png&controller=taco-kandi&audio=tab
// Live jam editor:
// http://localhost:6969/jam.html?shader=redaphid/taco/direct-kick&image=images/taco.png&controller=taco-kandi&audio=tab
// Seeded variants (each phone gets a unique look):
// http://localhost:6969/?shader=redaphid/taco/direct-kick&image=images/taco.png&controller=taco-kandi&seed=0.618&seed2=0.755&seed3=0.892&seed4=0.029
// http://localhost:6969/?shader=redaphid/taco/direct-kick&image=images/taco.png&controller=taco-kandi&seed=0.236&seed2=0.373&seed3=0.51&seed4=0.647
// http://localhost:6969/?shader=redaphid/taco/direct-kick&image=images/taco.png&controller=taco-kandi&seed=0.854&seed2=0.991&seed3=0.128&seed4=0.265
...
// Iter 71 (verified via 55s of recorded audio+pixel data, 132bpm track,
// 81% kick coverage but mean_vis_dLum only 0.016 — visible amplitude was
// too low because RAW_INTENSITY was peaking at ~0.7 and cubic ease maps
// 0.7 → ~0.33 of output). Two fixes:
//
// 1. Tightened smoothstep upper bound 0.95 → 0.65 so audio peaks easily
//    saturate. RAW_INTENSITY 0.65 now → output 1.0 → cubed 1.0 (was 0.07).
// 2. Added DIRECT_KICK channel that BYPASSES the cubic — bassZ + energyZ
//    spikes punch through immediately for the kick punch. Cubic stays for
//    the smooth between-beat baseline.
//
// Recorded BPM 132 confirms the kick detector finds real beats; we just
// weren't visibly reacting strongly enough. After this fix, peak kicks
// should produce mean_vis_dLum > 0.05 (3x current).
...
#define EASED_INTENSITY (smoothstep(0.15, 0.65, RAW_INTENSITY) * smoothstep(0.15, 0.65, RAW_INTENSITY) * smoothstep(0.15, 0.65, RAW_INTENSITY))
// DIRECT_KICK: instant peak detector that bypasses smoothstep+cubic so
...
// Iter 77: seed phase offsets — each device's plasma drifts at the same
// 0.05-0.13 Hz frequencies but at a different PHASE, so a group of kandi
// bracelets in the same room reads as related-but-distinct organisms.
// fract(seed) * TAU gives full 0..2π phase offset per axis. Bounded — the
// frequencies are unchanged, only the starting phase differs per device.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (interior > 0.05) {
        ...
        // Iter 76 fix: was CORONA_HUE - 0.6 = ~3.5 rad which I miscommented
        // as cyan but is actually purple-magenta territory. Pixel sampling
        // confirmed (#3a0054 = deep purple-magenta). Moved to CORONA_HUE - 1.2
        // = 2.8-3.2 rad = TRUE cyan, mathematically separated from magenta.
        vec3 r2_col = oklch2rgb(vec3(0.74, 0.16, CORONA_HUE - 1.2));  // true cyan
