...
// http://localhost:6969/?shader=redaphid/taco/seamless&image=images/taco-stencil.png&controller=taco-kandi
// Tab-audio mode (Spotify/SoundCloud — Chrome desktop):
// http://localhost:6969/?shader=redaphid/taco/seamless&image=images/taco-stencil.png&controller=taco-kandi&audio=tab
// Live jam editor:
// http://localhost:6969/jam.html?shader=redaphid/taco/seamless&image=images/taco-stencil.png&controller=taco-kandi&audio=tab
// Seeded variants (each phone gets a unique look):
// http://localhost:6969/?shader=redaphid/taco/seamless&image=images/taco-stencil.png&controller=taco-kandi&seed=0.618&seed2=0.755&seed3=0.892&seed4=0.029
// http://localhost:6969/?shader=redaphid/taco/seamless&image=images/taco-stencil.png&controller=taco-kandi&seed=0.236&seed2=0.373&seed3=0.51&seed4=0.647
// http://localhost:6969/?shader=redaphid/taco/seamless&image=images/taco-stencil.png&controller=taco-kandi&seed=0.854&seed2=0.991&seed3=0.128&seed4=0.265
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ---- VJ DRIP (from the-coat-23 lines 728-749) — knob_15 ----
    // Liquid droplet descends from taco center on bass hits. Smooth SDF, no grain.
    // Auto-wired iter 20 — user's knob_15=0.307 has been held set for 4+ ticks.
    if (knob_15 > 0.01) {
    ...
    // Single very thin halo right at the silhouette edge for definition.
    {
        ...
        float edgeBoost = 0.7 + drop_glow * 0.5;
    ...
    if (hasHistory) {
        // Per-frame inward pull: small (0.5%-2%), keeps echo fluid but readable.
        float echo_pull = 0.006 + knob_16 * 0.008 + bass_smooth * 0.004;
        // Direction: from current pixel toward taco center
        vec2 toward_center = normalize(taco_center_uv - uv + vec2(1e-5));
        vec2 sample_uv = uv + toward_center * echo_pull;
        // Read previous frame at the inward-shifted position
        vec3 echo_prev = getLastFrameColor(sample_uv).rgb;
        // Decay so the echo fades over distance (not feeding to white)
        float echo_decay = 0.88 + knob_16 * 0.08;
        echo_prev *= echo_decay;
        // Hue-age toward corona blue-violet (cosmic family)
        vec3 echo_lch = rgb2oklch(max(echo_prev, vec3(0.001)));
        echo_lch.z = mix(echo_lch.z, CORONA_HUE, 0.04);
        echo_prev = oklch2rgb(echo_lch);
        // Composite: ONLY outside the silhouette (don't smear over the logo itself)
        col = mix(col, max(col, echo_prev), (1.0 - silhouette) * (0.5 + knob_16 * 0.4));
    }
    ...
    if (knob_14 > 0.01 && hasHistory) {
        ...
        vec2 jUV = (uv - 0.5) * 2.0;
        jUV.x *= res.x / res.y;
        // Julia constant on a slow circle — c = 0.7885 * (cos t + i sin t) is the
        // classic spiraling Julia (moody-octopus2 line 37-38). Audio nudges it.
        float jt = iTime * 0.08 + bass_smooth * 0.4;
        float cRe = sin(jt) * 0.7885 + (spectralRoughnessNormalized * 0.005);
        float cIm = cos(jt) * 0.7885 + (spectralCentroidNormalized * 0.005);
        // Iterate the Julia formula a few steps — too many = over-warp, too few = no detail.
        // 4 iterations gives soft fractal tendrils without over-stretching.
        const int jSteps = 4;
        ...
        // Map iterated point back to UV space (small contribution — full warp is too much).
        vec2 sample_offset = jUV * 0.005 * knob_14;
        vec2 jSampleUV = uv + sample_offset;
        // Sample previous frame at the warped location
        vec3 julia_prev = getLastFrameColor(jSampleUV).rgb;
        // Decay so we don't feed back to white
        julia_prev *= 0.86;
        // Hue-anchor toward CORONA_HUE so it stays in the cosmic family
        vec3 jLCH = rgb2oklch(max(julia_prev, vec3(0.001)));
        jLCH.z = mix(jLCH.z, CORONA_HUE, 0.05);
        julia_prev = oklch2rgb(jLCH);
        // ONLY composite outside the silhouette (logo stays clean)
        col = mix(col, max(col, julia_prev), (1.0 - silhouette) * knob_14 * 0.7);
    }
