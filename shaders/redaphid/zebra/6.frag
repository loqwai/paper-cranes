...
#define variety 0.0003 + (time/10000.)
// Map PROBEs to knobs initially
#define PROBE_A float(int(time / 1.) % 10000) + (bassNormalized * energyNormalized) // Base rotation
...
{
  vec2 uv = fragCoord/iResolution.xy - 2.* vec2(bassNormalized/100., trebleNormalized/100.) - vec2(energyMax/100.) ;
  uv.x *= iResolution.x/iResolution.y;

  vec3 col = mandala_main(sin(uv*100.*(mapValue(bassNormalized,0.,1.,0.1,0.3)/100.)));
