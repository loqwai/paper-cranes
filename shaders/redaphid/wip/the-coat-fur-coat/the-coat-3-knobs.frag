// @fullscreen: true
// @mobile: true
// the-coat-3-knobs: knob-controlled version of the-coat-3
// All audio features replaced with knob controls for manual tweaking
...
// ============================================================================
// KNOB MAP — every audio feature is now a knob
//
// knob_1:  energy (overall intensity)         default ~0.5
// knob_2:  zoom (0=wide, 1=tight)             (kept from original)
// knob_3:  bass level                         default ~0.4
// knob_4:  bass spike (replaces bassZScore)   default 0.0 (0=calm, 1=hit)
// knob_5:  treble spike                       default 0.0
// knob_6:  camera tilt swagger                (kept from original)
// knob_7:  mids level                         default ~0.4
// knob_8:  spectral centroid (brightness)     default ~0.5
// knob_9:  feedback/trails (0=crisp, 1=smear) (kept from original)
// knob_10: spectral flux (timbral change)     default ~0.3
// knob_11: spectral roughness (grit)          default ~0.3
// knob_12: spectral entropy (chaos)           default ~0.4
// knob_13: spectral kurtosis spike            default 0.0
// knob_14: pitch class (note color)           default ~0.5
// knob_15: energy spike (replaces energyZScore) default 0.0
// knob_16: bass momentum (slope*r²)           default 0.0
// knob_17: energy momentum (slope*r²)         default 0.0
// knob_18: drop intensity                     default 0.0
// knob_19: beat pulse (tap to 1, release to 0) default 0.0
// knob_20: groove                             default ~0.4
// ============================================================================

// --- Knob-derived aliases (drop-in replacements for audio features) ---
#define K_ENERGY_NORM      knob_1
#define K_BASS_NORM        knob_3
#define K_BASS_Z           (knob_4 * 2.0 - 1.0)
#define K_TREBLE_Z         (knob_5 * 2.0 - 1.0)
#define K_MIDS_NORM        knob_7
#define K_CENTROID_NORM    knob_8
#define K_FLUX_NORM        knob_10
#define K_ROUGHNESS_NORM   knob_11
#define K_ENTROPY_NORM     knob_12
#define K_KURTOSIS_Z       (knob_13 * 2.0 - 1.0)
#define K_PITCH_NORM       knob_14
#define K_ENERGY_Z         (knob_15 * 2.0 - 1.0)
#define K_BASS_MOMENTUM    knob_16
#define K_ENERGY_MOMENTUM  knob_17
#define K_DROP             knob_18
#define K_BEAT             (knob_19 > 0.5)
#define K_GROOVE           knob_20

// --- ZOOM ---
#define BASE_ZOOM (mix(0.3, 2.5, knob_2) + K_ENERGY_NORM * 0.4)

// --- COAT SHAPE ---
#define SHOULDER_Y      (-0.02 + max(K_BASS_Z, 0.0) * 0.015)
#define SHOULDER_SPREAD (0.158 + K_BASS_NORM * 0.02)
#define SLEEVE_RADIUS   (0.04 + max(K_TREBLE_Z, 0.0) * 0.015)
#define SHOULDER_CAP     0.042
#define CHEST_W_BASE    (0.12 + K_BASS_NORM * 0.03)
#define CHEST_H_BASE    (0.065 + K_BASS_NORM * 0.01)
#define FUR_THICK       (0.06 + clamp(K_ENERGY_Z, 0.0, 1.0) * 0.04)
#define VNECK_WIDTH     (0.10 + clamp(K_ENERGY_Z, 0.0, 1.0) * 0.06)
#define VNECK_BOTTOM    (-0.013 - clamp(K_ENERGY_Z, 0.0, 1.0) * 0.03)

// --- MOTION ---
#define PUMP (K_BASS_NORM * 0.6 + clamp(K_BASS_MOMENTUM, 0.0, 0.5) * 3.0)
#define BEAT_PHASE (time * 2.2)
#define SNAP (clamp(K_TREBLE_Z, 0.0, 1.5))
#define HIP_SWAY (sin(time * 1.1))
#define GROOVE (K_MIDS_NORM * 0.8 + K_CENTROID_NORM * 0.2)

// --- DROP DETECTION ---
#define BUILD (clamp(K_ENERGY_MOMENTUM * 10.0, 0.0, 1.0))
#define IS_DROP clamp(BUILD + smoothstep(0.6, 1.0, K_ENERGY_Z) * 0.5, 0.0, 1.0)
#define DROP_TRIGGER_THRESH 0.8
#define SUSTAIN_GAIN 1.2

// --- DROP VISUALS ---
#define DROP_ZOOM 0.9
#define GODRAY_INTENSITY (clamp(K_ENERGY_Z, 0.0, 1.5) * 2.5 + clamp(K_TREBLE_Z, 0.0, 1.0) * 1.0)
#define EYE_WASH_STRENGTH (clamp(K_ENERGY_Z, 0.0, 1.0) * 0.5 + clamp(K_BASS_Z, 0.0, 1.0) * 0.2)
#define INTENSITY_ZOOM (K_ENERGY_NORM * 0.3)

// --- COLOR ---
#define HUE_BASE (0.78 + K_PITCH_NORM * 0.15)
#define HUE_DROP (0.08 + K_CENTROID_NORM * 0.1)

// --- COAT SURFACE ---
#define FLUFF_AGITATION (clamp(K_ENERGY_Z, 0.0, 1.0) * 0.5 + K_ROUGHNESS_NORM * 0.5)
#define BASS_PULSE_AMT (clamp(K_BASS_Z, 0.0, 1.0) * 0.8)
#define DROP_COLOR_AMT clamp(IS_DROP + clamp(K_ENERGY_Z, 0.0, 1.0) * 0.4 + clamp(K_BASS_Z, 0.0, 1.0) * 0.2, 0.0, 1.0)
#define GLEAM_INTENSITY (0.15 + K_FLUX_NORM * 0.35)

// Fractal fur
#define FUR_TRIGGER clamp(K_ENTROPY_NORM * 0.7 + K_ROUGHNESS_NORM * 0.5 + clamp(K_KURTOSIS_Z, 0.0, 1.0) * 0.2, 0.0, 1.0)
#define WARP_SPEED (K_CENTROID_NORM * 0.6 + K_FLUX_NORM * 0.2)
#define RIM_BOOST (2.5 + K_FLUX_NORM * 2.0 + clamp(K_BASS_Z, 0.0, 1.0) * 1.0)

// --- DERIVED ---
#define DROP_TRIGGER clamp(max(K_ENERGY_Z, BUILD), 0.0, 1.0)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float drop_hit = max(drop_now, max(drop_glow, K_DROP));
    ...
    float tilt_angle = (sin(time * 0.7) * 0.15
                     + (K_CENTROID_NORM - 0.5) * 0.2
                     + K_BASS_Z * 0.075) * knob_6;
