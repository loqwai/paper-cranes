...
#define HUE_BASE (fract(0.78 + time * 0.003 + pitchClassNormalized * 0.08 + midsNormalized * 0.10 + knob_3 + pitch_change * 0.06))
// Drop shifts toward hot orange/yellow
#define HUE_DROP (fract(0.99 + spectralCentroidNormalized * 0.05 + knob_3))
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // knob_4: fur fluff amplitude — 0=glass-edge 0.5=normal 1=explosive
    float k4_curve = pow(clamp(knob_4, 0.0, 1.0), 1.4);
    float fluff_amp = (0.018 + pump * 0.015 + SNAP * 0.012 + FLUFF_AGITATION * 0.02) * mix(0.05, 3.5, k4_curve);
    ...
    // VJ_HOUSE_BOUNCE_71: house-pocket chest pump (Party Jumpin' / CID territory).
    // Region: bassN > 0.55, mids 0.35..0.65, energyN > 0.55, low entropy (smooth).
    // Beat-locked lavender pulse on the body chest — does NOT touch the coat.
    {
        float vj_house_gate = smoothstep(0.50, 0.75, bassNormalized)
                            * smoothstep(0.30, 0.45, midsNormalized) * smoothstep(0.70, 0.55, midsNormalized)
                            * smoothstep(0.50, 0.75, energyNormalized)
                            * smoothstep(0.55, 0.30, spectralEntropyNormalized);
        float vj_house_thump = beat ? 1.0 : (0.4 + 0.6 * abs(sin(time * 5.5)));
        vec3 vj_lavender = vec3(0.78, 0.62, 1.0);
        col += chest_glow * vj_lavender * vj_house_gate * vj_house_thump * 0.55 * (1.0 - coat);
    }
    ...
    // knob_7: coat-rim chrome amplitude — 0=ghosted 0.5=normal 1=blazing
    float rim_boost = RIM_BOOST * mix(0.3, 2.0, knob_7);
    ...
    // knob_17: eye blaze — 0=embered, 0.5=normal, 1=supernova
    float k17_eye = mix(0.3, 3.0, clamp(knob_17, 0.0, 1.0));
    col += eyes * hot * 2.2 * k17_eye;
    ...
    // knob_16: god-ray feeling — 0=barely-there 0.5=normal 1=biblical
    col += god_rays * ray_col * GODRAY_INTENSITY * mix(0.2, 3.0, knob_16);
