...
// Fur puffs up with energy — knob_8 cranks it further
// knob_8: fur thickness (0=normal, 1=massive puffball)
#define FUR_THICK       (0.06 + clamp(energyZScore, 0.0, 1.0) * 0.04 + knob_8 * 0.08)
...
// knob_9: fluff wildness (0=smooth edge, 1=crazy bristles)
#define FLUFF_AGITATION (clamp(energyZScore, 0.0, 1.0) * 0.5 + spectralRoughnessNormalized * 0.5 + knob_9 * 1.5)
...
// knob_13: fractal fur visibility (0=audio-driven, 1=always full)
#define FUR_TRIGGER clamp(spectralEntropyNormalized * 0.7 + spectralRoughnessNormalized * 0.5 + clamp(spectralKurtosisZScore, 0.0, 1.0) * 0.2 + knob_13, 0.0, 1.0)
// Warp speed — knob_14 cranks the fractal fur swirl
// knob_14: fur swirl speed (0=audio-only, 1=hyperdrive)
#define WARP_SPEED (spectralCentroidNormalized * 0.6 + spectralFluxNormalized * 0.2 + knob_14 * 0.8)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Screen shake — only on extreme moments (drop_hit near max)
    float shake_gate = smoothstep(0.7, 1.0, drop_hit);
    float shake_amt = shake_gate * 0.005;
    uv.x += shake_amt * sin(time * 73.0);
    uv.y += shake_amt * cos(time * 91.0);
