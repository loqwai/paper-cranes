// LATTICE-BEAD (5.frag - FORK of 4.frag on 2026-09-04) - THE CHROMADEPTH BEAD.
// Built to be looked at through ChromaDepth 3D glasses, where the lens is a prism and HUE IS
// PARALLAX: red bends least and lands nearest, violet bends most and lands furthest. 4.frag stays
// the Oklch legible bead; ?cdoff=1 here reproduces it exactly.
//
// WHY THIS FORK IS SMALL, AND WHY IT WORKS. 4.frag had just solved figure/ground - a flat interior,
// a drawn contour, a receded gap - and a solved figure/ground IS a depth structure. It only needed
// to be spoken in the language the glasses read:
//     bead interior  -> NEAR -> red         contour rim -> MID -> green
//     lattice + gap  -> FAR  -> blue/violet
// So the geometry, the audio mappings, the relief, the rim light, the specular, the aurora and the
// onset flare are all UNTOUCHED. What changes is what they are allowed to SAY.
//
// THE ONE STRUCTURAL IDEA: HUE IS SPENT ENTIRELY ON DEPTH, AND EVERYTHING ELSE BECOMES EXPOSURE.
// In 4.frag each light source spoke in its own hue and the frame's colour was their sum. Summing
// hues is exactly what a ChromaDepth lens reads as a THIRD, WRONG depth. So the whole pipeline runs
// unchanged, then collapses to ONE SCALAR - a per-pixel exposure - and is repainted at the depth's
// hue. Nothing about the music is lost; it all arrives as light, which is where the standing
// channel hierarchy wanted it anyway (light takes all the audio; colour follows the slowest thing
// there is, and here the slowest thing is geometry itself).
//
// THE DEPTH AXIS IS gSD - the bead's own signed distance, normalised by the cell radius. Three
// other candidates were tried first and MEASURED degenerate on this shader, which is the finding
// worth keeping:
//     alpha    1.000 at every percentile   (the coarsest fold level covers the whole frame)
//     lum      ~1.02 from p25 to p98       (flat - cannot order anything)
//     ldFront  0.000 at every percentile   (same cause as alpha)
// and all three carry audio, which cannot be let into hue. gSD is pure geometry, defined over the
// whole frame, continuous, and monotone in the exact quantity the eye is already reading.
//
// TWO TREATMENTS, one lever:
...
//                      HOT RED FRESNEL contour. The silhouette of a round thing is the part of it
//                      nearest your eye, so painting that red throws the edge at the viewer.
//
// MEASURED, NOT ASSERTED (scripts/chromadepth-measure.mjs; ?cddebug=1 renders the depth field so
// the rendered hue can be checked against it. 512x512, all 11 mon, both modes, 262k px each):
//   Spearman rho(depth, hue)      0.9941 (shelf) / 0.9956 (dome)   - monotonic, per pixel
//   mean |hue - depth*0.75 - seed2*0.03|   0.0009                  - i.e. 8-bit quantisation
//   near-white fraction (HSL sat < 0.2)    0.000%   vs 0.384% for the same frame at ?cdoff=1
//   band mean hue   bead 0.034   rim 0.324   lattice 0.531   ground 0.711
// White is unreachable BY CONSTRUCTION, not by tuning: lightness is capped at 0.55 and saturation
// floored near 0.92, so the largest channel hsl2rgb can emit here is l + s - l*s = 0.964.
//
// WHAT IS FORBIDDEN HERE THAT 4.frag ALLOWS. paletteShift, regionHue, theme, melodyFlow,
// bassNoteFlow, pitchClassMean and the sectionMode glide all still run - they still shape the
// LIGHT - but not one of them reaches hue. In 4.frag rotating the palette is the reward for going
// hard; here it would silently invert the 3D across half the frame.
//
// THE BEAT POP, AND WHERE THE TWO DOCTRINES DISAGREE. The ChromaDepth guide says shift hue toward
// red on a beat so the frame pops forward. Read literally that is a global per-beat colour move,
// which this shader family has rejected four separate times. It is resolved by MASKING THE POP TO
// THE BEAD: the beads punch toward you and the lattice holds still. Spatially structured, so it
// cannot strobe; a better effect than the global version; and driven by onsetEnvelope, not `beat`
// (measured per-frame jitter 0.0087 vs 0.3072 for energyZScore, and `beat` clocks ~280 BPM against
// a 125 BPM set). Subtracted from depth and CLAMPED, never wrapped, so the nearest things saturate
// at red instead of rolling over into violet and inverting. Verified to fire and decay: mean depth
// 0.5051 at rest -> 0.4850 at full onset -> 0.5013 by 300 ms.
//
// NOT YET CONFIRMED THROUGH ACTUAL GLASSES. Everything above is measurable and was measured; that
...
// ==== PRESETS - CHROMADEPTH, WEAR THE GLASSES ====
// The recognition recipe from 4.md is a HARD REQUIREMENT here, not a suggestion: knob_169=0.60,
// navZoom=0.14, legible=1, knob_168=1.0. ChromaDepth needs a figure to put in front of a ground,
// and those four are what produce one. knob_168=0 turns the bead grid OFF, which leaves nothing
// for the near band to be, and the frame goes flat violet. ?wavelet=true and
// ?onset_refractory_ms=380 carry over from 3.frag for exactly the reasons they always did.
//
// theme= and paletteShift= are deliberately ABSENT. Both rotate lush()'s hue, and in chromadepth
// hue belongs to depth alone, so a palette preset here would be a preset for breaking the 3D.
// -- SHELF (cdmode=0): each bead is a flat plate standing off the lattice. The cleanest read. --
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-suhama.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=suhama%20%28three-mound%20sandbar%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ogi.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=ogi%20%28folded%20fan%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikyo.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=kikyo%20%28bellflower%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ume.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=ume%20%28plum%20blossom%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-katabami.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=katabami%20%28wood%20sorrel%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=tomoe%20%28comma%20and%20tail%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-mokko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=mokko%20%28melon%20quatrefoil%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=kikko%20%28tortoise-shell%20hexagon%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=kiku%20%28chrysanthemum%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=matsukawa%20%28pine-bark%20lozenge%29%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=hakkaku%20%28eight-pointed%20star%29%203D
// -- DOME (cdmode=1): each bead is a round thing with a HOT RED fresnel edge. More volume. --
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-suhama.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=1&name=suhama%20dome%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ogi.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=1&name=ogi%20dome%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikyo.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=1&name=kikyo%20dome%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ume.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=1&name=ume%20dome%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-katabami.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=1&name=katabami%20dome%203D
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=1&name=tomoe%20dome%203D
// -- the A/B: cdoff=1 is 4.frag exactly - same frame, Oklch palette, no depth encoding --
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/7&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikyo.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&cdpop=0.5&cdmode=0&name=kikyo%20flat%20%28cdoff%29&cdoff=1
// ==== END PRESETS ====
...
// @mobile: true
// @favorite: true
// @tags: fractal, hex, lattice, touch, color, chromadepth, 3d, redaphid
...
// ══ CHROMADEPTH (5.frag) ════════════════════════════════════════════════════════
// Levers. None of these has an "unset" state: 0 is a real, reachable value for every
// one of them, which is the trap 4.md caught LVK() falling into with ?legible=0.
uniform float cdoff;    // ?cdoff=1  -> chromadepth OFF, 4.frag's Oklch palette, byte-identical
uniform float cdmode;   // ?cdmode=0 SHELF (bead = a plate standing off the lattice)
                        // ?cdmode=1 DOME  (bead = a round thing, with a HOT RED fresnel rim)
uniform float cdpop;    // ?cdpop=0..1 how far an onset throws the bead forward. DEFAULT 0 = off,
                        // which is the conservative reading of the channel hierarchy; the presets
                        // below all pass 0.5. 0 has to stay reachable, so this is max(), not LVK().
uniform float cddebug;  // ?cddebug=1 render the DEPTH FIELD as greyscale, for measurement
// CD_ON is a SWITCH, not a fader. A partial crossfade between the Oklch palette and the
// depth palette is a blend of two different hues at one depth, which is precisely the
...
// none of them has a meaningful zero except CD_TEX, whose zero is reachable as a small epsilon.
#define CD_EXPO LVK(knob_183, 1.90, knob_183 * 4.0)
#define CD_GAMMA 0.85
// ── DEPTH BANDS. 0 = NEAR = red ... 1 = FAR = violet, and hue = depth * 0.75.
// The bands are chosen so the four things in frame land on four hues a naked eye can
// separate BEFORE the glasses go on - that is the honest test that the encoding exists.
//   bead interior   0.02..0.20  -> hue 0.015..0.150   red -> orange
//   contour rim     0.34..0.47  -> hue 0.255..0.353   yellow-green -> green
//   lattice lines   0.56..0.76  -> hue 0.420..0.570   green-cyan -> blue
//   empty ground    0.82..0.97  -> hue 0.615..0.728   blue -> violet
#define CD_BEAD_N 0.00
#define CD_BEAD_F 0.17
#define CD_FRES   0.01
#define CD_RIM_N  0.34
#define CD_RIM_F  0.47
#define CD_LAT_N  0.56
#define CD_LAT_F  0.76
#define CD_GND_N  0.82
#define CD_GND_F  0.97
// CD_GAP: how far OUT of the silhouette, in cell radii, the ground has fully receded to violet.
// Fitted, not guessed: measured on this frame the bead's signed distance runs -0.35 .. +0.50
// (percentiles p2..p98), so 0.40 spends the whole far band across the actual gap instead of
// saturating in the first few pixels.
#define CD_GAP  LVK(knob_186, 0.40, knob_186)
// CD_TEX: a BOUNDED hue wiggle from the lattice's own geometry, so the ground is textured
// rather than a flat poster. Bounded is the operative word - it is small enough that a pixel
// can never leave its own band, which is what keeps "hue = depth" true.
#define CD_TEX  LVK(knob_187, 0.030, knob_187 * 0.12)
// Saturation stays high and lightness is CAPPED at 0.55, which puts white out of reach by
// construction: the biggest channel hsl2rgb can emit here is l + s - l*s = 0.964. White has
// no hue, so a white pixel is a pixel with no depth - the failure mode this bounds away.
#define CD_SAT  0.97
#define CD_LMIN 0.07
#define CD_LMAX 0.55
// CD_SEED. The guide says shift the mapping with seed2 for per-device uniqueness. It has to be
// SMALL: at 0.07 with this device's seed2 = 0.755 the offset was 0.053, which is most of the red
// band, and the measured bead-band hue came back 0.097 - a bead that should read as NEAREST was
// painted orange. 0.03 keeps every device's near band inside red.
#define CD_SEED 0.03
...
// ── CHROMADEPTH PALETTE ─────────────────────────────────────────────────────────
// HSL, deliberately NOT the Oklch lush() above. ChromaDepth glasses are a prism: what
// they act on is the RAW SPECTRAL ORDER red->green->blue, so perceptual uniformity is
// not just unnecessary here, it actively bends the mapping the lens is expecting.
//
// HUE IS A FUNCTION OF DEPTH AND NOTHING ELSE. No paletteShift, no regionHue, no melody
// tilt, no theme, no sectionMode glide - every one of those exists in this shader and
// every one of them is excluded from this function on purpose. The moment any of them
// rotates the hue, "red = near" stops being true and the 3D inverts in places. seed2 is
// allowed exactly one bounded offset (0.07 max) because it is CONSTANT per device: it
// slides the whole map together, so the ORDER survives and each bead still owns a look.
vec3 cdpal(float t, float lit){
    t = clamp(t, 0.0, 1.0);
    float hue = t * 0.75 + seed2 * CD_SEED;          // max 0.82 - cannot wrap past violet
    float sat = clamp(CD_SAT - t * 0.05, 0.0, 1.0);  // slight falloff: far reads airier
...
vec3 seedLayer(vec2 p, vec2 c, float k, float pitch, float aaBase, float rimW){
    ...
    // .z (5.frag): the SIGNED distance, normalised by the cell radius so it is scale-free.
    // cov and rim are both smoothstepped, i.e. saturated - they say WHETHER you are inside,
    // never HOW FAR. ChromaDepth needs the how-far: it is what gives the bead a face
    // instead of a flat cutout. Negative inside, 0 on the silhouette, positive outside.
    return vec3(smoothstep(aa, -aa, d), smoothstep(aa * rimW, 0.0, abs(d)),
                clamp(d / max(pitch * 0.5, 1e-5), -1.0, 1.0));
}
...
vec4 fractal(vec2 p, out float ldFront, out float mFront){
    float scale = 1.0, aliasBase = 1.0 / iResolution.y;
    float alpha = 0.0, lumAcc = 0.0, fieldAcc = 0.0, waveAcc = 0.0;
    // CHROMADEPTH (5.frag): the shallowest recursion level that actually covers this pixel,
    // kept SEPARATELY from the coverage-weighted `field` average below. This distinction is the
    // whole ballgame: `field` is a mean over every level that contributed, and a mean over
    // depths lands in the MIDDLE of the gradient, which paints the entire frame green and
    // kills the pop (docs/advanced-shader-techniques.md section 3, "Do NOT average depth").
    // 1.0 = nothing in front of you, i.e. as far as this shader can see.
    ldFront = 1.0;
    // mFront: the front-most level's own distance to the nearest drawn line, normalised by the
    // cell radius. This is the LATTICE's depth. Taken from `m` and deliberately not from `lum`,
    // `alpha` or the composed colour: those three all SATURATE (measured on this frame - alpha
    // reads 1.000 at every percentile and lum > 4 everywhere, so neither can order anything) and
    // all three carry the audio, which would put the music back into HUE - the one channel
    // chromadepth cannot lend it. `m` is geometry. On a line = near; deep in a gap = far.
    mFront = 1.0;
    float ldFound = 0.0;
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        f *= mix(mix(1.0 - ld * 0.90, 0.10 + ld * 0.90, gDepthFocus), 1.0, gLevelOpen);
        // FRONT-MOST wins outright - first level with real coverage claims the pixel and no
        // later level can pull it back. The smoothstep is one antialiasing band wide, so this
        // is a discrete choice with a clean edge, not a blend.
        float ldHit = smoothstep(0.08, 0.20, f);
        float mN = clamp(0.5 + 0.5 * m / max(gHexR, 1e-3), 0.0, 1.0);   // CENTRED: m is signed and mostly negative, so the naive one-sided normalisation clipped 75% of the frame to exactly 0 (measured).
        ldFront = mix(ldFront, ld, (1.0 - ldFound) * ldHit);
        mFront  = mix(mFront,  mN, (1.0 - ldFound) * ldHit);
        ldFound = max(ldFound, ldHit);   // vj7-b7 COMPLEXITY RATCHET: the level WINDOW dissolves as the set clock advances, so deeper and shallower generations join the picture together instead of one band at a time — the structure literally gains detail over the night. Bounded (<=0.45) so the finest levels can never fully take over into speckle.   // iter 27: stronger window (screenshot: zoomed-out finest levels read as noise)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    gCov = 0.0; gRim = 0.0; gSD = 1.0;
    float ldFront, mFront;
    vec4 fr = fractal(uv, ldFront, mFront);
    ...
    // Scaling toward BLACK is hue- and saturation-safe (it is a uniform scale of the
    // triple), which is why the chromadepth branch below can reuse it unchanged.
    float vig = dot(sp, sp) * 0.38;                          // iter 27: real vignette — gives the eye a centre (the "focus" the user asked for, from composition not an overlay)

    if (CD_ON < 0.5){
        // ── 4.frag, unchanged ──
        col = mix(prev.rgb * 0.90, col, 0.82);               // gentle trail (low decay so the glow carries between frames)
        col = mix(col, vec3(0.0), vig);
        fragColor = vec4(clamp(col, 0.0, 1.0), 1.0);
    } else {
        // ══ CHROMADEPTH ═══════════════════════════════════════════════════════════
        // Everything above this line stays exactly as 4.frag computed it - the fold, the
        // seed grid, the relief, the rim light, the specular, the aurora, the onset flare.
        // What changes is what those things are ALLOWED TO SAY. In 4.frag they each spoke
        // in their own hue and the frame's colour was their sum. Here hue is spent entirely
        // on DEPTH, and the whole of 4.frag's audio work arrives as ONE SCALAR - exposure.
        // That is not a loss: it is the only arrangement in which the glasses work, and it
        // happens to agree with the standing channel hierarchy (light takes all the audio;
        // colour follows the slowest thing there is - here, geometry).
        //
        // THE DEPTH STRUCTURE IS 4.frag's OWN. LEGIBLE already separated figure from
        // ground: a flat interior, a drawn contour, a receded gap. Those three are handed
        // straight to the three depth bands, which is why this fork is small.

        // ── FRONT-MOST COMPOSITION, never averaged ────────────────────────────────
        // Each layer is selected through a mask that is binary except for one antialiasing
        // pixel. Coverage-weighted blending of two depths produces a THIRD hue that reads
        // as a depth nothing in the scene is at - the "green mush" failure.
        // THE DEPTH AXIS IS THE BEAD'S OWN SIGNED DISTANCE, gSD. Three candidates were tried
        // first and all three were measured DEGENERATE on this shader:
        //   alpha   - 1.000 at every percentile (the coarsest fold level covers the whole frame)
        //   lum     - ~1.02 from p25 to p98, i.e. flat
        //   ldFront - 0.000 at every percentile, for the same reason as alpha
        // and all three carry audio, which cannot be allowed into hue. gSD has none of those
        // problems: it is pure geometry, defined over the entire frame, continuous, and monotone
        // in exactly the quantity the eye is already reading as figure and ground.
        float sd      = clamp(gSD, -1.0, 1.0);

        // OUTSIDE the silhouette: leave the contour and recede across the gap to violet.
        float outD    = mix(CD_LAT_N, CD_GND_F, smoothstep(0.0, CD_GAP, sd));

        // THE BEAD. `inside` is 0 on the silhouette and grows toward the middle of the motif.
        float inside  = clamp(-sd, 0.0, 1.0);
        float rr      = clamp(1.0 - inside * CD_DOME, 0.0, 1.0);
        float domeZ   = sqrt(max(0.0, 1.0 - rr * rr));       // spherical cap: the bead has a FACE
        float shelfZ  = smoothstep(0.0, 0.11, inside);       // a flat plate with a thin bevel. Steep on purpose: measured, the bead interior only reaches inside = 0.35, so a 0.22 ramp left most of the figure short of the near band and reading orange.
        float beadD   = mix(CD_BEAD_F, CD_BEAD_N, mix(shelfZ, domeZ, CD_MODE));
        float covM    = smoothstep(0.35, 0.65, gCov);
        float depth   = mix(outD, beadD, covM);

        // THE CONTOUR. SHELF leaves it in the MID band, so the bead reads as a plate
        // standing off the lattice with a green edge - the cleanest figure/ground read.
        // DOME makes it a HOT RED FRESNEL instead: the silhouette of a round thing is the
        // part of it nearest your eye, and painting that red throws the edge at the viewer
        // (docs/advanced-shader-techniques.md section 3, "make it pop", (b)).
        float rimM    = smoothstep(0.28, 0.70, gRim);
        ...
        depth = clamp(depth + (gBeadOff + 0.5) * CD_BEADVAR * covM * (1.0 - rimT * tunnel), 0.0, 1.0);
        ...
        // Masked off the bead so the figure stays one clean plate.
        depth = clamp(depth + (mFront - 0.5) * 2.0 * CD_TEX * (1.0 - covM), 0.0, 1.0);

        // ── BEAT POP, SPATIALLY MASKED ────────────────────────────────────────────
        // The chromadepth guide says shift hue toward red on a beat so the frame pops
        // forward. Taken literally that is a GLOBAL per-beat colour move, which this
        // shader family has rejected four separate times. Masking it to the bead makes it
        // spatially structured - the beads punch toward you and the lattice holds still,
        // which is a better effect AND cannot strobe the frame.
        //
        // onsetEnvelope, not `beat`: measured per-frame jitter 0.0087 vs energyZScore's
        // 0.3072, and `beat` clocks ~280 BPM against a 125 BPM set because it stays true
        // while flux is high. Needs ?onset_refractory_ms=380. Subtracting from depth and
        // CLAMPING (never fract) means the nearest things saturate at red instead of
        // wrapping around to violet, so the ordering survives the pop.
        float cdPop = onsetEnvelope(0.012, 0.16) * clamp(onsetStrength * 1.8, 0.0, 1.0) * QGATE;
        depth = clamp(depth - CD_POP * 0.075 * cdPop * max(covM, rimM), 0.0, 1.0);

        if (cddebug > 1.5){
            // ?cddebug=2 - the RAW INPUTS to the depth field, so the band edges can be fitted to
            // this shader's actual distributions instead of guessed. R = lattice coverage alpha,
            // G = structure brightness lum (scaled), B = front-most recursion level.
            fragColor = vec4(clamp(0.5 + sd * 0.5, 0.0, 1.0), clamp(mFront, 0.0, 1.0), clamp(gCov, 0.0, 1.0), 1.0);
        } else if (cddebug > 0.5){
            // ?cddebug=1 - the depth field itself, so the encoding can be MEASURED against
            // the rendered hue rather than asserted. Bypasses the trail on purpose.
            fragColor = vec4(vec3(clamp(depth, 0.0, 1.0)), 1.0);
        } else {
            // ── EXPOSURE ──────────────────────────────────────────────────────────
            // MEAN of channels, not luminance. Luminance is hue-weighted (blue counts for
            // 0.07, green for 0.72), so with paletteShift permanently rotating lush()'s hue
            // over a set, a luminance read would make the frame drift brighter and darker
            ...
            e *= mix(1.0, eBead * covM + (1.0 - covM) * 0.50, tunnel);
            ...
            // FEEDBACK IN THE EXPOSURE DOMAIN ONLY. Mixing the previous frame's RGB would
            // drag last frame's HUE into this pixel, i.e. paint it at a depth it is not at,
            // and a moving camera would leave a trail of wrong depths behind every bead.
            // Reading back only its LIGHTNESS keeps the glow between frames and throws the
            // stale depth away. Decays (0.90) per the standing rule.
            float pe = clamp((rgb2hsl(prev.rgb).z - CD_LMIN) / max(CD_LMAX - CD_LMIN, 1e-3), 0.0, 1.0);
            e = clamp(mix(pe * 0.90, e, 0.82), 0.0, 1.0);

            vec3 cd = cdpal(depth, e);
            cd = mix(cd, vec3(0.0), vig);
            fragColor = vec4(clamp(cd, 0.0, 1.0), 1.0);
        }
    }
}
