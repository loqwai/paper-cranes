// @fullscreen: true
// @mobile: false
// moire-eye-6 — fork of moire-eye-5 with a MONOTONIC SPIN. The whole eye (iris anatomy + tendrils
// + moiré tunnel) now rotates forward-only via the controller's accumulated `spin_phase` uniform —
// audio sets the RATE (via bass), never the value, so it can't twitch forward-and-back the way the
// old iTime*audio rotation did. Requires &controller=moire-eye. knob_47 = spin speed (0 = frozen).
//
// ============================================================================
...
uniform float spin_phase;   // MONOTONIC spin from controllers/moire-eye.js (forward-only; rate set by bass + knob_47)
...
void mainImage( out vec4 C, in vec2 U )
{
    ...
    // MONOTONIC SPIN matrix — rotate COORDINATES by spin_phase (bounded cos/sin) rather than adding
    // the giant accumulated phase before high-freq multiplies (that would lose float precision).
    // The 0.5 constant gentles the default rate (controller's knob_47 still sets the underlying speed).
    float sp  = spin_phase * 0.5;
    float scs = cos(sp), ssn = sin(sp);
    mat2 spinM = mat2(scs, -ssn, ssn, scs);
    ...
    for (; i++<1e2;)
    {
        ...
        p.xz *= rh;
        p.yz *= rv;
        p.xy *= spinM;              // MONOTONIC spin of the moiré tunnel (controller-accumulated)
        r = length(p);
    ...
    vec2 ecFixed = ec;                      // unspun coord — for the fixed catchlight glint (a reflection doesn't spin)
    ec = spinM * ec;                        // MONOTONIC spin — rotate the whole anatomy coordinate together
    float er = length(ec);
    ...
    vec2 clp = ecFixed - vec2(-0.09, 0.11);   // fixed glint, doesn't rotate with the spin
    ...
    float twist = 3.0 + spectralFluxNormalized*6.0*REACT + energyNormalized*3.0;   // curl amount from music (bolder)
    float armN  = 6.0 + floor(spread*10.0);                                        // arm count from harmonic spread
    float wave  = sin(er*9.0 - iTime*0.6) * (0.5 + energyNormalized*0.8);          // radius-dependent BEND -> curly arms
    float curl  = (ang + wave*0.4)*armN + er*twist*6.2832;   // spiral + bend; rotation now MONOTONIC via spin_phase in ang
    float arms  = pow(0.5 + 0.5*sin(curl), 2.0);                                   // bolder ribbons (softer than spikes)
    float arms2 = pow(0.5 + 0.5*sin(curl*0.5 + 1.7 + wave), 2.0);                  // 2nd octave bends too (fractal)
    float tendril  = mix(arms, arms*arms2, 0.5);
    float tendZone = smoothstep(0.46, 0.54, er) * exp(-(er - 0.46)*1.5);           // reach FURTHER toward the corners
    // colour the arms (was flat blue-grey): a slow rainbow keyed to angle + centroid, rotating with the spin
    vec3 tendHue = H(ang*0.4 + spectralCentroidNormalized*0.4) * 0.45 + vec3(0.08, 0.10, 0.16);
    outc += (c*1.8 + tendHue) * tendril * tendZone * (0.7 + bassNormalized*0.9*REACT);
