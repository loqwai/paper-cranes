...
// SPLIT (H3, §8 variant 2): bead on the FINE levels, hexagon on the COARSE ones.
// WHICH END OF i IS WHICH: `scale *= s` (s~2) every iteration, so the fold domain
// shrinks as i grows -> LOW i = COARSE (big cells), HIGH i = FINE (filigree).
// Confirmed by the level-window comment: gDepthFocus 0 -> "coarse levels dominate"
// weights (1 - ld), and ld = (i - FIRST)/(LEVELS-1-FIRST), so ld=0 at i=FIRST.
// SPLIT_AT is the ld cut: levels with ld >= SPLIT_AT take the bead, coarser ones
// keep the hexagon. 0 => bead everywhere (identical to unsplit), >1 => hex everywhere.
// GEOMETRY, so hand knob only - never a per-frame audio value.
#define SPLIT_AT (knob_162)
...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        // ld hoisted above the cell mix (it used to be declared below `f`) so the
        // split can gate BEAD_MIX per recursion level. Same expression, same value.
        float ld = float(i - FIRST) / float(LEVELS - 1 - FIRST);
        float beadLevel = step(SPLIT_AT, ld);   // 1 on the FINE levels, 0 on the COARSE ones
        float hexTerm  = hexDist(uv) - hexR_i;
        float cellD    = hexTerm;
        // PERF PROBE ONLY (split-branch.frag): identical output to split.frag, but the
        // bead texture fetch is SKIPPED on coarse levels instead of being multiplied by 0.
        // The branch is uniform over the whole draw for a given level (SPLIT_AT is a
        // uniform, ld is a compile-time-ish per-level constant), so it is coherent.
        if (beadLevel > 0.5 && BEAD_MIX > 0.0) cellD = mix(hexTerm, beadDist(uv, hexR_i), BEAD_MIX);
