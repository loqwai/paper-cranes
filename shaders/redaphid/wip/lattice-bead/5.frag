// LATTICE-BEAD (5.frag - FORK of 4.frag on 2026-09-04) - RHYTHM ARTICULATION.
// 4.frag stays THE LEGIBLE BEAD: it won recognition (a stranger, across a dark room, can NAME
// their own bead) and nothing here is allowed to spend that. 5 attacks the next thing: 4's
// rhythm response is a UNISON FLASH. onsetEnvelope() is a single scalar, so on every hit every
// bead contour in frame lights on the SAME frame. It is correctly shaped, it does not shudder,
// and it is still one big blink - a wall of beads behaving like one lamp.
//
// THE MOVE: THE FLASH BECOMES A WAVE. Each seed cell subtracts its OWN arrival time from the
// shared timeSinceOnset clock, so ONE onset crosses the lattice as a ripple. The beat is now
// articulated in SPACE. No new audio measurement was added to do this - it is the same event,
// redistributed. Three envelopes ride it:
//   SNAP   8ms /  75ms  -> the tick, on the bead CONTOUR (this replaces 4's flare)
//   BODY  30ms / 300ms  -> the swell, on the bead INTERIOR (redistribution, see below)
//   GHOST 60ms / 440ms  -> a SECOND wavefront at 54% of the speed = dispersion, so the ring
//                          visibly doubles and the echo smears out behind the strike
//
// FOUR RULES, A LOT OF COMPLEXITY (the point: emergent, not enumerated):
...
//   2. + a per-cell stagger from a STABLE hash (never fract(sin(x)*43758.5453) - see hash11)
//   3. the two seed octaves ripple at DIFFERENT speeds: the near one chatters with treble, the
//      far one heaves with bass, so the frame has depth in its RHYTHM as well as its light
...
//
// CONSTRAINTS HONOURED (each was paid for with a live failure - see lattice-vj/HANDOFF.md):
//   * NO z-score anywhere in the drive. The only magnitude term is onsetStrength, which is
//     LATCHED PER ONSET - an event measure, not a per-frame level.
//   * The interior term REDISTRIBUTES rather than adds: wv.y is centred on its measured spatial
//     mean (WAVE_DUTY) so it is zero-mean across the grid and the frame's mean luminance is
//     unchanged. Handing light to a moving minority of cells is a ratchet; its COUNTER-RATCHET
//     is in the same edit (asymmetric clamp +1.10/-0.55, and the 1.45x contour gain).
//   * Nothing touches GEOMETRY. The focus, the speeds and the scatter are all LIGHTING - they
//     decide which cell lights FIRST, never where any cell IS.
//   * Nothing touches the global multiplier. Every term is cov- or rim-masked.
//   * COLOUR still follows only the slow music. The ghost's +0.66 hue offset is a CONSTANT.
//   * The ring wrap is on the RADIUS, between DISCRETE cells - a spatial edge, never the
//     fract()-teleport of docs/advanced-shader-techniques.md section 2. Each cell's envelope
//     stays continuous in time.
//
// ARTIC (knob_181, or ?artic=0..1), default 0, and 0 REPRODUCES 4.frag EXACTLY - so the fork is
// A/B-able in one query param and the recognition claim can be re-checked against its own
// baseline. All presets below set artic=1. Levers: ?rippleSpeed ?rippleScatter ?rippleRing
// ?rippleGhost (knob_182..185).
//
// STILL NEEDS ?wavelet=true AND ?onset_refractory_ms=380. Without wavelet ALL SEVEN wavelet
// springs read exactly 0.000, so the depth-coherent speeds collapse to one speed. Without the
// refractory the detector free-runs at ~213 BPM chasing hats instead of locking to ~125, and the
// wave is retriggered before it has crossed the frame.
//
// NOT YET CONFIRMED BY A HUMAN. Measurements are in 5.md.
...
// LEGIBLE defaults to 0, which reproduces 3.frag EXACTLY - the texture look is not lost, it is
// one end of a dial. Opt in with ?legible=0..1 or knob_180.
//
// SIZE IS A PREREQUISITE, NOT A DETAIL. At the shipped knob_169=0.28 a seed cell is
// exp2(mix(-6,2,0.28)) = 0.074 in uv, i.e. tens of beads across the frame, each a few pixels
// wide. No amount of figure/ground separation makes a shape that small nameable. The presets
// below therefore raise knob_169 as well: recognition needs BOTH a big cell and a clean edge.
//
...
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-suhama.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=suhama%20%28three-mound%20sandbar%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ogi.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=ogi%20%28folded%20fan%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikyo.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=kikyo%20%28bellflower%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ume.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=ume%20%28plum%20blossom%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-katabami.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=katabami%20%28wood%20sorrel%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=tomoe%20%28comma%20and%20tail%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-mokko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=mokko%20%28melon%20quatrefoil%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=kikko%20%28tortoise-shell%20hexagon%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=kiku%20%28chrysanthemum%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=matsukawa%20%28pine-bark%20lozenge%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=hakkaku%20%28eight-pointed%20star%29
// -- mon x palette: the same five silhouettes in each curated palette --
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.45&name=hakkaku%20Deep%20Cyan
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=1.05&name=hakkaku%20Ember
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.75&name=hakkaku%20Violet
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=0.15&name=hakkaku%20Acid%20Lime
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.45&name=tomoe%20Deep%20Cyan
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=1.05&name=tomoe%20Ember
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.75&name=tomoe%20Violet
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=0.15&name=tomoe%20Acid%20Lime
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.45&name=kiku%20Deep%20Cyan
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=1.05&name=kiku%20Ember
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.75&name=kiku%20Violet
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=0.15&name=kiku%20Acid%20Lime
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.45&name=matsukawa%20Deep%20Cyan
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=1.05&name=matsukawa%20Ember
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.75&name=matsukawa%20Violet
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=0.15&name=matsukawa%20Acid%20Lime
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.45&name=kikko%20Deep%20Cyan
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=1.05&name=kikko%20Ember
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.75&name=kikko%20Violet
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=0.15&name=kikko%20Acid%20Lime
// -- the texture end of the dial: legible=0 reproduces 3.frag exactly --
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikyo.png&knob_161=1&knob_168=1.0&knob_169=0.28&legible=0&artic=1&navZoom=0.30&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=Kikyo%20texture%20%28legible%200%29
// -- 5.frag: RHYTHM ARTICULATION levers (rippleSpeed / rippleScatter / rippleRing / rippleGhost) --
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&rippleSpeed=0.20&rippleScatter=0.06&rippleRing=0.75&name=hakkaku%20SLOW%20RING%20%28clean%20concentric%20relay%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=1.05&rippleSpeed=0.82&rippleScatter=0.95&rippleRing=0.20&name=tomoe%20SPRAY%20%28fast%20ragged%20wavefront%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=1&paletteShift=0.75&rippleSpeed=0.46&rippleGhost=1.0&name=kiku%20GHOST%20%28double%20ring%2C%20echo%20forward%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.28&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&rippleSpeed=0.60&rippleRing=0.35&navZoom=0.30&name=kikko%20SWARM%20%28fine%20pitch%2C%20many%20rings%20in%20flight%29
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/5&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&artic=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=0.15&artic=0&name=matsukawa%20A%2FB%20%28artic%3D0%20is%204.frag%29
// ==== END PRESETS ====
...
// LIGHT is driven by discrete EVENTS rather than by measured levels.
//
// WHY THE FORK EXISTS. The run hit the shudder-vs-lag dilemma head-on: raw features shudder, and
// smoothing them costs about a second of lag (measured, factor 0.10). Every fix inside that trade
// just moves along it. onsetEnvelope() escapes it - a second AnalyserNode with
// smoothingTimeConstant = 0 feeds an edge-triggered `onset` (true for exactly ONE frame), and the
// shader SYNTHESISES its own response curve instead of inheriting the audio's shape.
//   drive jitter, per frame:  z-score 0.3072  ->  energySpring 0.0320  ->  onsetEnvelope 0.0087
//   tempo lock:               beat 280 BPM    ->  onset default 213    ->  +380ms refractory 125.3
// 125.3 BPM with a 3% gap spread is the actual tempo of the set. `beat` was counting hats.
//
// THE FLARE (the one thing 3 changes in spirit, already present at fork):
//   flareZ = onsetEnvelope(0.012, 0.16) * clamp(onsetStrength * 1.8, 0, 1) * QGATE * K179
// It rides the seed-grid RIM only. Light lane, never geometry, never the global multiplier.
//
// BAKED PRESET (live at fork: quietGate 0.634, sectionMode 6, evoPhase 0.829):
//   knob_161=1     BEAD_MIX      - full mon silhouette as the lattice cell
//   knob_168=0.9   seed amount   - the bead grid as LIGHTING over the fold
//   knob_169=0.28  seed pitch    - log-mapped; ~ one bead per fist at arm's length
//   knob_178 (QGATE floor, default 0.35) and knob_179 (flare gain, default 0.55) left at default
//   URL:  &autofly=1 &wavelet=true &onset_refractory_ms=380 &theme=0 &image=images/beads/mon-kikyo.png
//   controllers: wavelet-ease -> lattice-nav -> lattice-controls  (order matters, left fold)
//
// TWO PARAMS ARE NOT OPTIONAL:
//   ?wavelet=true         - without it ALL SEVEN wavelet springs read exactly 0.000, so
//                           bassLive/midsLive/trebLive are dead and most of the shader is deaf.
...
// LEGIBLE: a HAND lever, never audio. DEFAULT 0 = 3.frag reproduced EXACTLY; opt in with
// ?legible=0..1 or knob_180.
//
// It was first written as LVK(knob_180, LVK(legible, 0.55, legible), knob_180), i.e. the
// house "0 means unset, fall back to the baked default" convention. That convention CANNOT
// EXPRESS ZERO, so ?legible=0 silently became 0.55 and the two rendered bit-identically
...
// -- RHYTHM ARTICULATION (5.frag) -----------------------------------------------
// The levers for the wavefront. ARTIC follows LEGIBLE's shape exactly, and for the same
// reason: the house LVK(k, baked, map) convention means "0 = unset, use the baked
// default", which CANNOT EXPRESS ZERO - 4.frag's own header records that trap silently
// turning ?legible=0 into 0.55. ARTIC therefore has no unset state, so 0 is reachable
// and the claim "artic=0 reproduces 4.frag bit-for-bit" is actually testable. Every
// 5.frag preset sets artic=1 explicitly.
uniform float artic;          // ?artic=0..1          master lever. 0 == 4.frag.
uniform float rippleSpeed;    // ?rippleSpeed=0..1    wavefront speed (maps 2.5..12 cells/s)
uniform float rippleScatter;  // ?rippleScatter=0..1  per-cell stagger: clean ring vs ragged spray
uniform float rippleRing;     // ?rippleRing=0..1     radius wrap in cells (2.6..9.0)
uniform float rippleGhost;    // ?rippleGhost=0..1    echo gain
#define ARTIC (clamp(max(knob_181, artic), 0.0, 1.0))
// RK: knob OR url param, else the baked default. Unlike ARTIC these four DO keep the
// house "0 means unset" behaviour, because 0 is not a meaningful value for any of them
// (a zero-speed wavefront never arrives, a zero ring is a divide guard). Documented so
// nobody repeats the ?legible=0 investigation on them.
#define RK(k, u, baked) mix((baked), max((k), (u)), step(0.001, max((k), (u))))
// The measured spatial mean of the BODY envelope across the grid. The redistribution
// below is centred on it so it is zero-mean by construction; see the measurement note
// in 5.md (mean-luminance parity artic=0 vs artic=1).
#define WAVE_DUTY 0.26
...
// -- RHYTHM ARTICULATION: THE PER-CELL WAVEFRONT (5.frag) -----------------------
// The one idea this fork exists for. 4.frag's flare is a UNISON flash: onsetEnvelope()
// is a single scalar, so every bead in frame lights at the same instant and the grid
// reads as one big blink. Here each cell subtracts its OWN arrival time from the shared
// clock, so a single onset becomes a wave crossing the lattice and the beat is
// articulated in SPACE. Nothing new is measured from the audio to do this - it is the
// same event, redistributed.
// hash11 is defined further down (with the long float32-stability note that earned it).
// Forward-declared here so the wavefront can sit next to the seed grid it belongs to
// instead of being marooned 40 lines away from seedLayer.
float hash11(float p);

vec2 gFocusW;                       // where the current wave was born, in world uv
float gRing, gScatter, gSpdA, gSpdB;

// Stable per-cell hash. hash11 is bit-reproducible in float32 ONLY while its argument
// stays O(1) - see the long note above it, where the textbook fract(sin(x)*43758.5453)
// two-stated frame to frame and came out of the fold as a completely different lattice.
// A seed-grid cell index runs to several hundred once the lattice pans, so the index is
// folded onto a 97-cell torus FIRST. mod() of an integer-valued float is exact, and
// every argument below stays under ~40, so this cannot two-state. The 97-cell period is
// far larger than any framing that shows a nameable bead.
float cellHash(vec2 ci){
    vec2 m = mod(ci, 97.0);
    return hash11(m.x * 0.311 + hash11(m.y * 0.517 + 0.19) * 7.31);
}

// onsetEnvelope() with the clock SHIFTED per cell. Returns exactly 0 before the
// wavefront arrives (an un-hit cell is silent, not dark) and decays from there, so it
// inherits onsetEnvelope's whole reason for existing: the shader SYNTHESISES the
// response curve instead of inheriting the audio's shape, which is why the measured
// per-frame jitter is 0.0087 rather than a z-score's 0.3072. timeSinceOnset starts at
// 1000, so before the first onset every cell reads exp(-1000/release) = 0. No load flash.
float cellEnv(float delay, float attack, float release){
    float t = timeSinceOnset - max(delay, 0.0);
    if (t < 0.0) return 0.0;
...
// Per-cell wavefront for ONE seed octave. Cheap on purpose: a floor, a hash and three
// exp() - no beadDist and no texture fetch, so it costs a small fraction of seedLayer,
// which is why it can run twice (once per octave) without a frame-rate note.
//   x = SNAP  (8ms / 75ms)   the tick  - rides the contour
//   y = BODY  (30ms / 300ms) the swell - rides the interior
//   z = GHOST (60ms / 440ms) the echo  - a second, slower wavefront (see below)
vec3 seedWave(vec2 p, vec2 c, float k, float pitch, float spd){
    vec2 q  = (p - c) * k + c;                      // same q seedLayer folds
    vec2 ci = floor(q / pitch + 0.5);               // this octave's integer cell index
    vec2 fc = ((gFocusW - c) * k + c) / pitch;      // the SAME world point, on this octave's grid
    float rC = length(ci - fc);
    // Concentric expanding rings. Wrapping the RADIUS (not a phase) is what keeps the
    // wave bounded: without it a cell 40 cells from the focus would wait 6 seconds and
    // simply never articulate. It also means several rings are in flight at once, which
    // is where the depth comes from. The seam falls BETWEEN two discrete cells, so it is
    // a SPATIAL edge - each individual cell's envelope is still continuous in time and
    // nothing snaps frame to frame. (Contrast docs/advanced-shader-techniques.md section 2:
    // a fract() wrap on a CONTINUOUS travelling wave teleports; this one cannot.)
    float rW = mod(rC, max(gRing, 0.75));
    // THE STRIKE MUST LAND ON THE STRIKE. Measured twice, because the first fix was not
    // enough. With a bare rW/spd the nearest cell still sat ~0.37 cells from the focus and
    // the whole grid was dark for the first ~90 ms of every beat while the interior term
    // held its trough - the frame went DARKER on the kick and only bloomed afterwards,
    // which is precisely the inversion 4.frag's ground-recede notes warn about
    // (corr(energySpring, brightness) -0.68: a drop made the frame darker and flatter).
    // A -0.30 bias only cut the dead period to ~60 ms (still 3.6 frames of nothing, on a
    // shader whose whole onset design exists to avoid latency).
    // -0.75 is not a taste number: |ci - fc| for the NEAREST integer cell is at most
    // sqrt(2)/2 = 0.707 whatever fc is, so this GUARANTEES the cell under the focus fires
    // at delay 0 - at every framing, every pitch and every pan position.
    float r0 = max(rW - 0.75, 0.0);
    // The stagger is applied as a per-cell SPEED, not as an additive delay. Two reasons.
    // (a) An additive stagger can push a near cell late and re-open the dead period; a
    // multiplicative one is exactly 0 at the origin, so the strike stays tight and the
    // ring gets progressively more ragged as it spreads - which is also what a wavefront
    // through a disordered medium actually does. (b) It cannot make a delay negative, so
    // no cell can ever fire BEFORE the event that caused it. The factor is clamped so a
    // maxed-out scatter cannot invert it.
    float del = r0 * clamp(1.0 + (cellHash(ci) - 0.5) * gScatter, 0.18, 2.4) / max(spd, 0.25);
    // GHOST: the same hit propagating at ~54% of the speed and decaying ~6x slower.
    // Physically this is dispersion - one event, two propagation rates - so the ring
    // visibly doubles and the trailing copy smears out behind the strike.
    return vec3(cellEnv(del,                0.008, 0.075),
                cellEnv(del,                0.030, 0.300),
                cellEnv(del * 1.85 + 0.045, 0.060, 0.440));
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // -- RHYTHM ARTICULATION per-frame state (5.frag) -----------------------------
    // THE FOCUS is where each onset is born. It drifts on two INCOMMENSURATE slow clocks
    // (0.055 and 0.041 of bTime - ratio 1.3415..., irrational to the precision that
    // matters here) so its path is a Lissajous that never closes and the grid is never
    // struck the same way twice. Deliberately SLOW: the focus travels ~0.25 cells/s, so a
    // given cell's arrival time drifts by ~20 ms across one 480 ms beat gap, well under a
    // frame of visible smear. That matters because a cell's delay changing MID-FLIGHT is
    // the one way this design could shiver.
    // It is a LIGHTING parameter - it decides which cell lights FIRST, never where any
    // cell IS - so the geometry-only-evolves directive does not apply, and the sin/cos
    // here are not the forbidden geometry oscillators. Seeded per device (seed2/3/4).
    gFocusW = vec2(sin((bTime * 0.055 + flowPhase * 0.010 + seed3) * TAU),
                   cos((bTime * 0.041 + morphPhase * 0.008 + seed2) * TAU)) * (0.55 + 0.45 * seed4);
    gRing   = mix(2.6, 9.0, clamp(RK(knob_184, rippleRing, 0.42), 0.0, 1.0));
    // SCATTER is driven by spectralEntropyMedian - a MEDIAN, so section-scale, and from a
    // different DOMAIN (spectral quality) than the onset that fires the wave or the
    // treble/bass that pace it. Decorrelation directive: the wave's trigger, its speed and
    // its shape are three independent signals, not one signal drawn three times.
    // A tonal passage rings cleanly; a noisy one sprays. One rule, a lot of variety.
    // Range chosen for the MULTIPLICATIVE form in seedWave: this is +-gScatter/2 on each
    // cell's wave speed, so 1.5 means the slowest cells arrive ~4x later than the fastest.
    // Past ~1.9 the factor would cross zero and the arrival order would invert, which is
    // why seedWave clamps it as well - a range guard on a term two functions away from
    // where it is used is exactly the kind of thing that survives until it is live.
    gScatter = clamp(RK(knob_183, rippleScatter, 0.55), 0.0, 1.0) * 1.5
             * (0.40 + 0.95 * clamp((spectralEntropyMedian - 0.55) * 2.2, 0.0, 1.0));
    // DEPTH-COHERENT SPEEDS. bandForDepth() already lights near recursion levels with
    // treble and far ones with bass; the two seed octaves now get the same treatment in
    // TIME. The near octave chatters fast with the top end, the far one heaves slowly with
    // the low end, so one hit produces two ripples at two spatial rates and the frame has
    // depth in its rhythm as well as in its light.
    // Both audio terms are wavelet SPRINGS (measured per-frame jitter 0.032 vs a z-score's
    // 0.307) and both are bounded to a fraction of the hand speed. This modulates a RATE,
    // never a position, and the multiplier is strictly positive, so a wavefront can be
    // slowed or hurried but can NEVER be run backwards.
    float spdBase = mix(2.5, 12.0, clamp(RK(knob_182, rippleSpeed, 0.46), 0.0, 1.0));
    gSpdA = spdBase * (0.90 + 0.50 * clamp(trebLive, 0.0, 1.0) * QGATE);
    gSpdB = spdBase * (0.40 + 0.30 * clamp(bassLive, 0.0, 1.0) * QGATE);
    ...
    if (knob_168 > 0.001){
        ...
        float xf = smoothstep(0.0, 1.0, zf);
        vec2  sl = mix(lA, lB, xf);
        // Same crossfade as the geometry, so the wave arrives in step with the octave it
        // belongs to instead of drifting off its own cells during the dive.
        vec3  wv = mix(seedWave(uv, world, kA, seedPitch, gSpdA),
                       seedWave(uv, world, kB, seedPitch, gSpdB), xf);
        float sd  = 0.0;
        ...
        // (corr(energySpring, brightness) -0.68: a drop made the frame DARKER and FLATTER).
        // -- ARTICULATION: REDISTRIBUTION, NOT ADDITION (5.frag) ------------------
        // `alive` is the only place the hit's MAGNITUDE enters. onsetStrength is latched
        // per onset - an EVENT measure, not a per-frame level - so nothing here is a
        // continuous z-score drive. QGATE'd, so a silent room articulates nothing and
        // bodyMod collapses to exactly 1.0, i.e. to 4.frag.
        float alive = clamp(onsetStrength * 1.8, 0.0, 1.0) * QGATE * ARTIC;
        // The bead-gain budget is NOT increased. It is handed to the cells the wavefront
        // is passing through and taken from the ones it is not. wv.y is centred on its
        // measured spatial mean (WAVE_DUTY) so the term is zero-mean across the grid by
        // construction and the frame's mean luminance is unchanged - which is precisely
        // what makes this legal under "never SUM energy-correlated terms into brightness".
        // Verified, not asserted: mean luminance at artic=0 vs artic=1 is compared in 5.md.
        // The clamp is deliberately ASYMMETRIC (+1.10 / -0.55) - the wavefront may brighten
        // about twice as hard as the trough may darken. Handing light to a moving minority
        // of cells is a ratchet on the frame's average, and that asymmetry plus the 1.45x
        // contour gain below are its COUNTER-RATCHET, in this same edit.
        // Clamp measured, not guessed. At -0.55 the trough dimmed every bead interior by
        // 53% for the whole stretch between rings, which read as the seed switching off
        // rather than as beads taking turns. -0.38/+1.25 keeps mean-luminance parity (see
        // 5.md) while leaving the un-hit cells clearly lit, so the silhouette - the thing
        // 4.frag exists to protect - never drops out of the frame.
        float bodyMod = 1.0 + alive * clamp((wv.y - WAVE_DUTY) * 2.35, -0.38, 1.25);
        col += lush(s, 0.95) * cov * knob_168 * (0.05 + 0.58 * pump) * bodyMod * mix(1.0, 1.9, LEGIBLE);
        ...
        //
        // 5.frag: THE FLASH BECOMES A WAVE. onsetEnvelope() is a single scalar, so 4.frag's
        // flare lit every contour in frame on the same frame - a unison blink. wv.x is the
        // same envelope with a per-cell delay, so the identical event now travels. Crossfaded
        // by ARTIC, so artic=0 is 4.frag's expression exactly and the fork is A/B-able.
        // The 1.45x on the wavefront side is not a gain grab: at any instant only a moving
        // fraction of contours are lit, so the same total light is spread over fewer pixels.
        float flareZ = mix(onsetEnvelope(0.012, 0.16), wv.x * 1.45, ARTIC)
                     * clamp(onsetStrength * 1.8, 0.0, 1.0);
        flareZ *= QGATE;
        col += lush(s + 0.33, 1.0) * rim * knob_168 * flareZ * LVK(knob_179, 0.55, knob_179);
        // -- GHOST ECHO -----------------------------------------------------------
        // The second wavefront from the same hit (seedWave.z): 54% of the speed, ~6x the
        // release. The ring visibly doubles and the trailing copy smears out behind the
        // strike. It carries a FIXED +0.66 hue offset - a CONSTANT, not an audio drive, so
        // colour still follows only the slow music - which makes the echo read as a
        // different colour behind the leading edge rather than as a dimmer copy of it.
        // THIS IS THE ONE TERM IN THE FORK THAT ADDS rather than redistributes, so it is
        // the one that could blow out. It is rim/cov-masked (spatially structured, never a
        // global multiplier), gated by `alive` (so silence adds nothing), and bounded well
        // under the leading edge's amplitude. Blown-pixel fraction vs 4.frag is measured in
        // 5.md rather than argued.
        float ghost = wv.z * alive * clamp(RK(knob_185, rippleGhost, 0.50), 0.0, 1.0);
        col += lush(s + 0.66, 1.0) * (rim * 0.55 + cov * 0.20) * knob_168 * ghost * 0.50;
    }
