...
vec3 lush(float s, float lit){
    ...
    float L = clamp(0.33 + EXB(knob_156, 0.40) + 0.40 * clamp(lit, 0.0, 1.0), 0.05, 0.92);   // K156 LIGHT BASE: the palette's baseline lightness (the tuned 0.33 was five iterations of 'less washed out').   // vj2 iter 6: 0.40+0.44 → 0.33+0.40 (max L 0.84 → 0.73). Meter on lit passages: dark 2.4 %, lum 0.30, sat 0.85 — pastel. Lower L keeps chroma, restores the floor. (Oklch: hue untouched.)   // MUTED (iter 17, from chromadepth-lattice/3): lower base lightness
...
// ---------------------------------------------------------------- H9: WHOLE MOTIF
// MEASURED status quo (scripts/lab-probe.mjs, GPU readback, controller off):
//   at the coarsest drawn level the texture coordinate spans 0.494 .. 2.165
//   = 1.671 texture periods, and 89% of pixels sample OUTSIDE [0,1].
//   min == 0.494 ~= 0.5 is the damning half: because fractal() passes `abs(p)`,
//   the coordinate NEVER goes below the texture centre, so only the motif's
//   upper-right QUADRANT is ever fetched, mirrored into four, and then a whole
//   further tile is dragged in by REPEAT wrap. A 12-lobe rosette cannot be a
//   rosette under that transform, whatever the rosette looks like.
//
// Two independent levers, selected by knob_164 so each can be scored alone:
//   L1 UNMIRROR  - sample on the SIGNED fold coordinate instead of abs(p).
//                  Free: hexDist() takes abs() internally, so hexDist(abs(p))
//                  is IDENTICAL to hexDist(p) -- the hex baseline cannot move.
//                  The cross term delt2 keeps abs(p) and is likewise untouched.
//   L2 ONE TILE  - clamp the lookup to the tile (in-shader CLAMP_TO_EDGE; the
//                  texture's own wrap mode is shared state in Visualizer.js and
//                  must not be edited) and EXTEND the field analytically outside
//                  it, so the exterior keeps growing monotonically instead of
//                  wrapping. A bare clamp would plateau and can flood; the
//                  `length(tc - tcc) * 2.0` term is the standard SDF-outside-a-box
//                  extension, in q units (q = 2 * texcoord units).
//
//   knob_164:  0.00 OFF (byte-identical to 1.frag)   0.25 L1 only
//              0.50 L2 only                          0.75 L1 + L2
#define WHOLE_UNMIRROR (step(0.15, knob_164) * step(knob_164, 0.35) + step(0.65, knob_164))
#define WHOLE_ONETILE  (step(0.40, knob_164))

float beadDist(vec2 pSigned, vec2 pMirror, float r){
    vec2 src = mix(pMirror, pSigned, WHOLE_UNMIRROR);
    vec2 q   = src / max(r, 1e-3);       // bead-normalised space
    vec2 tc  = q * 0.5 + 0.5;            // [-1,1] -> texture coords
    vec2 tcc = clamp(tc, 0.0, 1.0);
    vec2 use = mix(tc, tcc, WHOLE_ONETILE);
    float d  = (getInitialFrameColor(use).g - 0.5) * 2.0 * BEAD_RANGE;
    d += length(tc - tcc) * 2.0 * WHOLE_ONETILE;   // monotone exterior, q units
...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        float beadTerm = beadDist(p, uv, hexR_i);
        {   // PROBE: the coordinate beadDist() actually fetches, with the levers applied
            int selLvl = FIRST + int(floor(knob_163 * 5.0 + 0.5));
            if (i == selLvl){
                vec2 src = mix(uv, p, WHOLE_UNMIRROR);
                vec2 tc  = (src / max(hexR_i, 1e-3)) * 0.5 + 0.5;
                vec2 use = mix(tc, clamp(tc, 0.0, 1.0), WHOLE_ONETILE);
                pMaxT = max(use.x, use.y);
                pMinT = min(use.x, use.y);
                pWrap = (use.x < 0.0 || use.x > 1.0 || use.y < 0.0 || use.y > 1.0) ? 1.0 : 0.0;
            }
        }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    fragColor = vec4(clamp((pMaxT + 2.0) / 6.0, 0.0, 1.0),
                     clamp((pMinT + 2.0) / 6.0, 0.0, 1.0), pWrap, 1.0);
}
