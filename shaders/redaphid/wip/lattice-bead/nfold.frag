...
// ============================ H13: SYMMETRY-MATCHED FOLD =====================
// WAVE-1 TERMINAL FINDING: the drawn cell sits at p = 0, which is the CORNER of
// the mirror-repeat fold's output domain -- the point where BOTH triangle waves
// turn around. Two mirror lines therefore pass through every single cell, so the
// lattice can only ever display the motif's D2 projection. kiku (12-fold), ume
// (5-fold) and katabami (3-fold) are structurally unable to survive it.
//
// H13 changes TWO things inside fractal():
//   (a) CELL SITE  -- the cell moves off the fold's corner to the MID-RAMP point
//       p = 1 - s/4, where the triangle wave is a pure similarity (locally a
//       translation + scale, no reflection). A cell placed there carries the
//       motif WHOLE; its symmetry becomes ours to choose instead of the fold's.
//   (b) N-FOLD KALEIDOSCOPE -- the cell coordinate is then folded into a TAU/N
//       wedge whose mirror axis is +y, which is a true mirror axis of every mon
//       (japanese.py builds every rosette with phase=90, i.e. a petal at the top).
...
#define BEAD_CY   (step(0.0005, knob_169) * (knob_169 - 0.5) * 0.4)

// N-fold kaleidoscope about the origin, mirror axis on +y.
// n < 2 is a no-op, so knob_166 = 0 leaves the coordinate untouched.
vec2 kaleidoN(vec2 q, float n){
    if (n < 1.5) return q;
    float w = TAU / n;
    float a = atan(q.y, q.x) - PI * 0.5;      // measure from the petal axis
    a = abs(a - w * floor(a / w + 0.5));      // fold to [0, w/2], mirrored
    a += PI * 0.5;
...
float beadDist(vec2 p, float r){
    ...
    q += vec2(BEAD_CX, BEAD_CY);         // H13: bbox centre -> rotational centre
...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        vec2 pf = p;                                          // H13: fold output BEFORE the per-level twist
        ...
        // ── H13 CELL SITE ──────────────────────────────────────────────────
        // legacy: uv = abs(p) -> the cell sits ON the fold's two mirror lines.
        // H13   : the cell sits at the MID-RAMP of the triangle wave, where the
        //         fold is a pure similarity, then gets exactly the symmetry we
        //         ask for from kaleidoN(). Same rot2 convention as `p *= rot2()`.
        vec2 uvL = abs(p);
        vec2 cl  = (pf - vec2(1.0 - 0.25 * s)) * rot2(theta);
        vec2 uv  = mix(uvL, kaleidoN(cl, KAL_N), CELL_SITE);
        ...
        float delt2 = min(length(uvL) - gCross, min(uvL.x, uvL.y)) + gCrossBias; // BASS taut cross (+K139 hex↔cross balance)
