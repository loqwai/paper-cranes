...
// SOLID_MIX: 0 = outlined cell (abs(cellD - gRingGap)), 1 = solid cell (cellD - gRingGap).
// GEOMETRY, like BEAD_MIX — hand knob or one-way eased step only, never per-frame audio.
#define SOLID_MIX (knob_163)
// SOLID_FIX: 1 = keep level COVERAGE on the ring so solid fill cannot occlude the
// recursion, and apply the fillW lit-area counter-ratchet. 0 = naive solid.
#define SOLID_FIX (knob_162)
...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        // H4 (lab/fill): OUTLINED vs SOLID cell, the one code path from HANDOFF §3.
        // raw = cellD - gRingGap.  abs(raw) -> m is small only on a RING at offset
        // gRingGap from the cell boundary => an OUTLINED cell.  raw alone is negative
        // everywhere INSIDE that boundary, and both the line smoothstep (gBorder) and
        // the fill smoothstep (gFill) run "high -> low", so every interior pixel
        // saturates => a SOLID cell.  SOLID_MIX=0 is byte-identical to 1.frag.
        float rawD  = cellD - gRingGap;
        float ringD = abs(rawD);                          // OUTLINE distance
        // COUNTER-RATCHET (K162, on by default). Solid changes lit area ~3x, and the
        // reason is NOT exposure: `w = (1-alpha)*f` composites levels FRONT TO BACK, so
        // a saturated coverage `f` on the coarsest level drives alpha to 1 and OCCLUDES
        // every deeper level. Keeping coverage on the ring while letting only the FILL
        // go solid preserves the recursion; fillW then restores mean luminance inside
        // the fill threshold (local, per-depth) instead of the forbidden global mult.
        float delt1 = mix(mix(abs(rawD), rawD, SOLID_MIX), ringD, SOLID_FIX);   // K144 RING GAP: distance from the hex ring to the drawn line — sets how HOLLOW each cell is, independently of its radius.        // MIDS breathe the hexagons
        float delt2 = min(length(uv) - gCross, min(uv.x, uv.y)) + gCrossBias; // BASS taut cross (+K139 hex↔cross balance)
        float m = min(delt1, delt2);
        float mFill = min(mix(delt1, mix(ringD, rawD, SOLID_MIX), SOLID_FIX), delt2);   // fill lane distance
        float fillW = mix(1.0, mix(1.0, 0.30, SOLID_MIX), SOLID_FIX);                   // lit-area counter-ratchet
        ...
        float lit = (smoothstep(gFill + alias, gFill, mFill) * (0.5 + relief) * fillW + 0.18)
