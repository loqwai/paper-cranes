// LATTICE-BEAD (5.frag - FORK of 4.frag on 2026-09-04) - TIMBRAL COLOUR.
// 4.frag stays the legible bead: it won RECOGNITION (figure/ground via LEGIBLE) and nothing here
// touches that. 5 changes exactly one channel - COLOUR - and it changes what colour is FOR.
//
// THE BUG 4.frag COULD NOT SEE (measured, not guessed). lush()'s chroma was
//     C = (cb + seed2*cs) + ca * sin(s * TAU * 0.5 + 1.3)
// The hue is fract(s) - PERIOD 1. That chroma term has PERIOD 2. So the same hue renders at
// C 0.2109 (s=0) or C 0.1146 (s=1) - 1.84x apart - decided by nothing but where the unbounded
// accumulators (bTime, paletteShift, sectionMode) happen to sit. Consequences, all of them real:
//   * SATURATION WAS A HIDDEN SLOW OSCILLATOR. Nobody wired it. Over a set, paletteShift climbs
//     and the whole palette silently fades to pastel and back on a 2.0 cycle of paletteShift.
//     That is what "fuzzy terrible fuchsia" actually is: not a bad hue, a hue caught at C 0.113.
//   * PER-MATERIAL COLOUR WAS IMPOSSIBLE. lush(s + 0.33, 1.0) for the rim changed the rim's
//     CHROMA as an unavoidable side effect of changing its HUE. You could not ask for "same
//     colour family, more saturated edge" - the function had no way to express it.
// Verified numerically against a JS port of lush() before a line of the shader was changed; the
// full table is in 5.md.
//
// THE FIX, AND THE FEATURE. Chroma becomes an EXPLICIT ARGUMENT (lushC) and is driven by TIMBRE
// instead of by hue position. Four moves, all in the colour channel only:
//
//   1. TONALITY -> CHROMA.  clean/peaky spectrum = vivid, noisy/flat = grey.
//      TONALITY = spectralCrestSmooth (peakiness) - spectralEntropyMedian (chaos). Both are slow
//      by construction - one is the controller's smoothed crest, the other a ~500-frame median -
//      and both are listed INDEPENDENT OF ENERGY, so saturation now tracks the character of the
//      sound rather than how loud it is. In Oklch, moving C at fixed L barely moves luminance, so
//      this cannot pump the frame the way a brightness term would.
//
//   2. TWO HUE FIELDS AT INCOMMENSURATE RATES.  Periods 55.6 min and 90.0 min, ratio 1.6187
//      (golden), so their interference NEVER visibly repeats - the colour field keeps arriving
//      somewhere new with no accumulator to reset. Each field's phase is offset by one slow
//      timbral median (spectralSkewMedian / spectralKurtosisMedian, both SHAPE domain), so a
//      change of timbre re-lands the whole interference somewhere else in the pattern.
//      Worst-case hue drift 0.0315 turns/min - SLOWER than the bTime term already shipped
//      (0.04 turns/min) and 8x under the rate the user rejected as "flashing" (0.24).
//
//   3. MATERIAL, NOT TINT.  Timbre sets how far APART ground / body / rim sit on the wheel, not
//      what colour the frame is:
//        GROUND  hue split by GRAIN  (spectralRoughnessSmooth - QUALITY domain)
//        BODY    chroma by TONALITY, hue barely moves - the bead keeps its identity
//        RIM     hue split by WIDTH  (spectralSpreadMedian - SHAPE domain), most chroma of the three
//      A dissonant, wide-spectrum passage pushes the ground away from the bead and the frame reads
//      GRAINY; a clean narrow one collapses them and it reads GLAZED. Four different domains
//      across the four levers (quality / shape / tonal / quality) - not one signal drawn 4x.
//
//   4. THEME 4 "timbral".  Themes 0-3 are lifted verbatim from other lattice shaders and are left
//      byte-identical. Theme 3 measurably WASHES OUT (L slope 1.20 pins L at the 0.92 ceiling for
//      any lit > 0.683 - most of a lit bead), so 5 adds a fifth entry tuned for this shader
//      instead of editing a shared one: lower ceiling (0.80), wider chroma envelope.
//
// GAMUT IS SAFE BY CONSTRUCTION, NOT BY LUCK. The timbral chroma is expressed in the SAME UNITS
// and the SAME [-1,1] range as the sin() term it replaces, so on themes 0-3 the chroma envelope
// is bit-for-bit the one 4.frag already shipped - it is only ADDRESSED differently. Measured clip
// fractions for both builds are in 5.md.
//
// CONSTRAINTS HONOURED (each was paid for with a live failure):
//   * COLOUR FOLLOWS THE SLOWEST MUSIC ONLY - every new driver is a Median or a controller Smooth.
//     There is not one zScore, one raw feature, or one energy-correlated term in the colour path.
//   * No zScore anywhere in colour. No geometry changed AT ALL - the silhouette read that 4.frag
//     bought is untouched, and the recognition recipe below is 4.frag's, unmodified.
//   * Nothing added to the global multiplier; no term summed into brightness or alpha.
//   * Frame feedback decay unchanged (mix(prev * 0.90, col, 0.82)).
//   * ?flatcolour=1 (or knob_181) reproduces 4.frag EXACTLY. Following the ?legible=0 lesson, the
//     lever is named for the OFF state so that "absent" (uniforms default to 0) means ON and the
//     off value is genuinely reachable - the LVK "0 means unset" convention cannot express zero.
// ==== PRESETS ====
// Recognition recipe is 4.frag's, unchanged: knob_169=0.60 navZoom=0.14 legible=1 knob_168=1.0.
// Theme 4 is the new timbral palette; the rest are the curated palettes the sweep found strong.
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-suhama.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=suhama%20%28three-mound%20sandbar%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ogi.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=ogi%20%28folded%20fan%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikyo.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=kikyo%20%28bellflower%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-ume.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=ume%20%28plum%20blossom%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-katabami.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=katabami%20%28wood%20sorrel%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=tomoe%20%28comma%20and%20tail%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-mokko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=mokko%20%28melon%20quatrefoil%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kikko.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=kikko%20%28tortoise-shell%20hexagon%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=kiku%20%28chrysanthemum%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=matsukawa%20%28pine-bark%20lozenge%29%20timbral
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=4&paletteShift=1.35&name=hakkaku%20%28eight-pointed%20star%29%20timbral
// -- the palettes the sweep found strong, now with timbral chroma on top --
...
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-hakkaku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=hakkaku%20Jade
...
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-tomoe.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=tomoe%20Jade
...
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=kiku%20Jade
...
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=matsukawa%20Jade
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-matsukawa.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=0.15&name=matsukawa%20Acid%20Lime
// -- A/B: flatcolour=1 reproduces 4.frag exactly --
//https://visuals.beadfamous.com/?shader=redaphid/wip/lattice-bead/6&controller=wavelet-ease&controller=lattice-nav&controller=lattice-controls&image=images/beads/mon-kiku.png&knob_161=1&knob_168=1.0&knob_169=0.60&legible=1&navZoom=0.14&autofly=1&wavelet=true&onset_refractory_ms=380&fullscreen=true&theme=0&paletteShift=1.35&name=kiku%204.frag%20colour%20%28flatcolour%29&flatcolour=1
// ==== END PRESETS ====
...
uniform float theme;       // ?theme=0..4 selects a palette (4 = the new timbral one, 5.frag)
uniform float flatcolour;  // ?flatcolour=1 (or knob_181) restores 4.frag's colour EXACTLY.
                           // Named for the OFF state on purpose: uniforms default to 0, so
                           // "absent" means the timbral palette is ON and 0 is still reachable.
                           // The house LVK(k, baked, map) convention cannot express zero - that
                           // is the bug ?legible=0 hit in 4.frag, and this sidesteps it.
...
// ── TIMBRAL COLOUR LEVERS (5.frag) ────────────────────────────────────────────
// TCOLOUR 1 = timbral palette (default), 0 = 4.frag reproduced exactly.
#define TCOLOUR (1.0 - clamp(max(knob_181, flatcolour), 0.0, 1.0))
// TONALITY  -1 noisy/flat .. +1 clean/peaky.  QUALITY domain, both halves slow by construction:
//   spectralCrestSmooth  = the controller's smoothed peak/mean ratio
//   spectralEntropyMedian= ~500-frame median chaos estimate
// Neither is energy-correlated (see the independence matrix), so this is genuinely "what does the
// sound sound LIKE", not "how loud is it". Drives CHROMA only.
// CENTRE MATTERS MORE THAN GAIN. First cut centred entropy at 0.55, which is BELOW where music
// actually sits (spectralEntropyMedian runs ~0.75-0.85 on real material and the harness pins
// 0.8), so ordinary music read as "noisy" and the whole frame came out DESATURATED - measured
// sat 0.213 vs 4.frag's 0.322 on theme 3, i.e. the change made the washed-out problem WORSE.
// Centred on the real operating point, typical music sits at TONALITY ~ 0 and the timbre has
// room to swing both ways instead of being pinned at the grey end.
#define TONALITY (clamp((spectralCrestSmooth - 0.45) * 1.9 - (spectralEntropyMedian - 0.78) * 1.8, -1.0, 1.0))
// GRAIN  0..1 dissonance.  QUALITY domain, smoothed by the controller. Drives the GROUND hue split.
#define GRAIN    (clamp(spectralRoughnessSmooth * 1.8, 0.0, 1.0))
// WIDTH  0..1 harmonic width.  SHAPE domain, a median. Drives the RIM hue split.
#define WIDTH    (clamp(spectralSpreadMedian * 2.4, 0.0, 1.0))
// MATERIAL CHROMA, in ABSOLUTE Oklch units. This is not how it was first written and the reason
// is worth keeping: material chroma was originally expressed in the same ca-scaled units as the
// term it replaces, which looked elegant and did NOTHING - ca is only 0.04..0.07 on the stock
// themes, so a 0.26 spread between rim and ground came to 0.013 of actual chroma. Measured: moving
// those biases by 0.1 changed frame saturation by 0.003 while costing 0.6pt of gamut crush. The
// envelope (TONALITY) stays ca-scaled so it matches 4.frag's range exactly; the MATERIAL split has
// to be absolute or it cannot be seen.
#define MATC_BODY    (0.014)
#define MATC_RIM     (0.032)
#define MATC_GROUND (-0.030)
...
void themeConsts(float t, out float lb, out float ls, out float lo, out float hi, out float cb, out float cs, out float ca){
    ...
    else if (t < 3.5) { lb = 0.10; ls = 1.20; lo = 0.05; hi = 0.92; cb = 0.075; cs = 0.05; ca = 0.04; }
    // 4 timbral (5.frag). Themes 0-3 are lifted verbatim from other lattice shaders and stay
    // byte-identical; this is the one tuned for the timbral chroma. Theme 3's L slope of 1.20 pins
    // L at the 0.92 ceiling for any lit > 0.683 - i.e. most of a lit bead is FLAT WHITE, which is
    // the measured wash-out. Here the ceiling drops to 0.80 and the slope is gentle, so the bead
    // has somewhere to go, and ca widens to 0.065 to give TONALITY real travel.
    else              { lb = 0.42; ls = 0.38; lo = 0.06; hi = 0.80; cb = 0.150; cs = 0.05; ca = 0.070; }
}

// lushC - lush() with CHROMA AS AN ARGUMENT. This is the whole of 5.frag's colour change.
//
// 4.frag's chroma was `ca * sin(s * TAU * 0.5 + 1.3)`. Hue is fract(s), period 1; that term has
// PERIOD 2. Measured on a JS port of this exact function (theme 0, seed2 0.755):
//     s=0.000 hue   0deg C 0.2109      s=1.000 hue   0deg C 0.1146
//     s=0.125 hue  45deg C 0.2124      s=1.125 hue  45deg C 0.1131
// Identical hue, 1.84x the chroma, decided only by where bTime/paletteShift/sectionMode had
// accumulated to. Two things followed and both were shipped bugs:
//   * saturation was a slow oscillator nobody wired - the palette fades to pastel and back over
//     a 2.0 cycle of paletteShift, which is the "washed out" reading of the shipped default;
//   * per-material colour was inexpressible - lush(s + 0.33) for the rim moved the rim's chroma
//     as an unavoidable side effect of moving its hue, so "same family, hotter edge" was not
//     something the function could say.
//
// The ENVELOPE term keeps 4.frag's exact ca-scaled range and only changes what ADDRESSES it
// (TONALITY instead of hue position), so with TCOLOUR = 0 this is bit-identical to 4.frag and with
// TCOLOUR = 1 the envelope cannot open a gamut hole 4.frag did not already have. `matC` is the one
// genuinely new chroma, capped at 0.032 - measured cost in 5.md.
vec3 lushC(float s, float lit, float matC){
    float h = fract(s) * TAU;
    ...
    // In Oklch, moving C at fixed L barely moves perceived luminance - which is precisely why
    // timbre is allowed to drive chroma at all. The same wire into LIGHTNESS would be a brightness
    // pump and would be forbidden by the standing hierarchy.
    float C = max(0.0, (cb + seed2 * cs) + EXB(knob_157, 0.14))
            + ca * mix(sin(s * TAU * 0.5 + 1.3), TONALITY * 0.62, TCOLOUR)   // ENVELOPE: hue-locked -> timbre-locked, same range
            + matC * TCOLOUR;                                                // MATERIAL: absolute, and only ever a few hundredths
    C = max(C, 0.0);
    C *= LV_RICH * mix(1.0, 0.3 + knob_4 * 2.4, DIAL_ON(knob_4));   // dial 3 (knob_4): colour intensity
    return fitGamut(oklch2rgb(vec3(L, C, h)), L);
}
// Default material: every un-specialised call site still gets timbral chroma, and still collapses
// to 4.frag when TCOLOUR = 0.
vec3 lush(float s, float lit){ return lushC(s, lit, 0.0); }
...
// ── TIMBRAL HUE INTERFERENCE (5.frag) ──────────────────────────────────────────
// TWO slow hue fields, carried at INCOMMENSURATE rates, laid over each other. regionHue() above
// already makes position matter; this makes position matter DIFFERENTLY AS THE SET RUNS, and it
// makes different parts of the lattice sit in different "keys" at the same moment.
//
// bTime = iTime / 3, so the rates below are one full turn of field A every 55.6 min and of field B
// every 90.0 min. Their ratio is 1.6187 - close enough to the golden ratio that the interference
// between them has no short common period: the field keeps arriving somewhere it has not been,
// with no accumulator that can saturate or reset.
//
// RATE BUDGET (this is the part that gets shaders rejected, so it is computed, not eyeballed).
// Worst-case |d hue / dt| = sum over terms of A * TAU * (rate / 3) * freqMultiplier
//     field A: 0.09 * TAU * 0.0003    * (1 + 1.309) = 0.000392 turns/s
//     field B: 0.065 * TAU * 0.0001853 * (1 + 0.764) = 0.000133 turns/s
//     total                                          = 0.000525 turns/s = 0.0315 turns/min
// The bTime hue term 4.frag already ships runs at 0.04 turns/min, and the user rejected 0.24 as
// "the palette flashing". So this is SLOWER than what is already there and 8x under the failure.
//
// The PHASES are offset by two slow timbral medians from the SHAPE domain (skew = dark/bright
// tilt, kurtosis = peaked vs diffuse). They are offsets, not rates, so a timbre change re-lands
// the whole interference pattern somewhere else rather than accelerating it - the hue can never
// be made to run fast by the music, only to be somewhere else.
...
float timbreHue(vec2 w, float pa, float pb){
    return 0.088 * sin(w.x * 0.1730 + pa * TAU)
         + 0.088 * cos(w.y * 0.1090 + pa * TAU * 1.3090)
         + 0.064 * sin((w.x + w.y) * 0.0670 + pb * TAU)
         + 0.064 * cos((w.x - w.y) * 0.0413 + pb * TAU * 0.7639);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float s = field
            ...
            + timbreHue(world, tPhaseA(), tPhaseB()) * TCOLOUR;   // 5.frag: the two incommensurate slow hue fields

    // ── MATERIAL LANES (5.frag) ────────────────────────────────────────────────
    // Timbre sets how far APART the three surfaces sit, not what colour the frame is. That is the
    // difference between colour reading as MATERIAL and colour reading as a tint: a tint moves
    // everything together and you stop seeing it, a material split changes the RELATIONSHIP
    // between figure and ground and the eye keeps noticing.
    //
    // The four levers come from four different domains on purpose (constraint: bass+energy+flux
    // together is one signal drawn three times):
    //     GRAIN    spectralRoughnessSmooth   QUALITY   -> ground hue split
    //     WIDTH    spectralSpreadMedian      SHAPE     -> rim hue split
    //     TONALITY crest vs entropy          QUALITY   -> chroma of all three
    //     phases   skew / kurtosis medians   SHAPE     -> where the interference sits
    // and the base journey still carries pitchClassMean (TONAL) from 4.frag.
    //
    // Hue OFFSETS are small (<= 0.26 turns) and driven by medians, so nothing here can jitter:
    // the split breathes over tens of seconds as the arrangement changes, it does not track beats.
    float sGround = s + 0.5  + (GRAIN - 0.5) * 0.24 * TCOLOUR;   // dissonance pushes ground off the bead
    float sBody   = s        - (GRAIN - 0.5) * 0.05 * TCOLOUR;   // the bead barely moves - it must stay recognisable
    float sRim    = s + 0.33 + (WIDTH - 0.5) * 0.26 * TCOLOUR;   // a wide spectrum throws the contour further round

    // Chroma per material. The RIM carries the most so the drawn edge stays the most saturated
    // thing in frame (this HELPS the silhouette read that 4.frag bought); the GROUND carries the
    // least so it stays subordinate. Both are counter-ratchets on the body, which sits between.
    float cBody   = MATC_BODY;
    float cRim    = MATC_RIM;
    float cGround = MATC_GROUND;

    vec3 col = lushC(s, lum, cBody);                          // (brightness handled by the bloom below)
    col += lushC(s + 0.12 + 0.05 * sin(evoPhase * 0.7), 0.9, cRim) * wave * max(0.0, 0.6 + EXB(knob_158, 1.1));   // K158 ACCENT: strength of the travelling pulse accent.   // pulse accent (hue-shifted, brighter); the SET CLOCK swings the accent 0.08..0.28 off base over minutes
    ...
    //    + slow time. Intensity from treble spring + grit. Structural lighting, not colour flashing.
    {
        ...
        vec3  rimCol = lushC(sRim, 1.0, cRim);
        ...
        col += lushC(sRim, 1.0, cRim) * spec * (0.25 + trebLive * 0.6 + spectralCrestSmooth * 0.3 + WUB * 0.55) * QGATE;   // vj8-b11 LEARNED: wub also sharpens the SPECULAR glint, so a wobbly bassline makes the lattice edges glint hard and a clean passage lets them settle. Two channels for one feature = you can see it whether you are looking at the cells or the edges.
    ...
    //    with spectral BRIGHTNESS + GRIT (both smoothed, neither a phone fader). QGATE-calm.
    {
        ...
        vec3 acol = lushC(sGround + a.y * 0.15, 0.9, cGround);                       // complement, drifting up the screen
    ...
    if (knob_168 > 0.001){
        ...
        vec3 beadInk = lushC(sBody, mix(0.62, 0.80, pump), cBody);
        ...
        col += lushC(sBody, 0.95, cBody) * cov * knob_168 * (0.05 + 0.58 * pump) * mix(1.0, 1.9, LEGIBLE);
        col += lushC(sRim, 1.0, cRim) * rim * knob_168 * (0.22 + 0.45 * trebLive * QGATE) * mix(1.0, 1.6, LEGIBLE);
        // ── DROP FLARE (K179) ──────────────────────────────────────────────────
        ...
        flareZ *= QGATE;
        col += lushC(sRim, 1.0, cRim) * rim * knob_168 * flareZ * LVK(knob_179, 0.55, knob_179);
    }
