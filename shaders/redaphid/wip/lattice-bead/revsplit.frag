...
// ─────────────────────────── H8: THE REVERSE SPLIT (lab/revsplit) ───────────────────────────
// ORIENTATION (verified in this file, not assumed):
//   `scale *= s` runs BEFORE `if (i < FIRST) continue;`, so the fold domain shrinks as i grows
//   and `alias = aliasBase * 0.5 * scale` widens with i. => HIGH i = FINE. ld = (i-FIRST)/(LEVELS-1-FIRST)
//   so ld=0 at i=FIRST=4 (COARSEST, biggest on-screen cells) and ld=1 at i=9 (finest).
//   H3 (lab/split, verdict NO) put the BEAD on i=7,8,9 — the levels only a few px across, where a
//   silhouette degrades to speckle. H8 puts the BEAD on the COARSE levels i=4,5,6 instead.
//
// This is a REAL UNIFORM BRANCH, not a mix(). mix(hexTerm, beadTerm, W) evaluates BOTH operands,
// so beadDist()'s dependent texture fetch runs on every level regardless of W and the per-level
// saving of §9 is never realised (measured: all-hex/all-bead/split all 0.45-0.47 ms). The branch
// condition below is loop- and uniform-invariant, so the fetch is genuinely skipped on hex levels.
//
// SPLIT_MODE (knob_162), quantised into the four arms:
//   0.00  all-hex    bead on NO level                      (baseline)
//   0.25  all-bead   bead on EVERY drawn level             (== substrate BEAD_MIX=1)
//   0.50  fwd-split  bead on the FINE levels   i=7,8,9     (H3's losing arm, kept as control)
//   0.75  rev-split  bead on the COARSE levels i=4..       (H8 — the hypothesis)
// REV_SPAN (knob_163) picks the rev-split point: N = 1,2,3 coarse levels take the bead.
//   0.0-0.33 -> i=4 only | 0.34-0.66 -> i=4,5 | 0.67-1.0 -> i=4,5,6
// GEOMETRY CHANNEL: both knobs move where cell boundaries ARE. Hand knob / one-way eased step
// only — never a per-frame audio value (lattice-vj HANDOFF §2.1).
#define SPLIT_MODE (knob_162)
#define REV_SPAN   (knob_163)
...
float beadDist(vec2 p, float r){
    ...
    vec2 uv = q * 0.5 + 0.5;             // [-1,1] -> texture coords
    float d = (getInitialFrameColor(uv).g - 0.5) * 2.0 * BEAD_RANGE;
...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        // ── H8 REVERSE SPLIT: per-level BRANCH (see SPLIT_MODE header). Uniform-invariant, so
        //    the bead texture fetch is genuinely skipped on hexagon levels — unlike the mix() this
        //    replaces, which evaluated beadDist() on every level and saved nothing.
        int   drawn    = i - FIRST;                       // 0..5, 0 = COARSEST
        int   revN     = int(clamp(floor(REV_SPAN * 3.0), 0.0, 2.0)) + 1;   // 1..3 coarse levels
        bool  useBead;
        if      (SPLIT_MODE < 0.125) useBead = false;              // all-hex
        else if (SPLIT_MODE < 0.375) useBead = true;               // all-bead
        else if (SPLIT_MODE < 0.625) useBead = (drawn >= 3);       // fwd-split: FINE  i=7,8,9
        else                         useBead = (drawn <  revN);    // rev-split: COARSE i=4..
        float cellD;
        if (useBead) cellD = beadDist(uv, hexR_i);                 // texture fetch ONLY here
        else         cellD = hexDist(uv) - hexR_i;
