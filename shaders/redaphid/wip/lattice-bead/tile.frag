...
// ============================================================================
// H12 — TRANSLATION-REPEAT FOLD (lab/tile, 2026-09-03)
// ============================================================================
// The baseline fold is a TRIANGLE WAVE:
//     p = 1.0 - abs(s * fract(p - 0.5) - s * 0.5);
// With s = 2 it maps every axis into [0,1] and puts the drawn cell at p = 0, a
// CORNER of the folded tile. Two consequences, both fatal to "see your bead":
//   * p is never negative, so `abs(p)` at the sampling site is a no-op and the
//     bead texcoord tc = p/r*0.5+0.5 only ever reaches [0.5,1] -- ONE QUADRANT
//     of the bake, mirrored four ways. You literally cannot see 3/4 of the mon.
//   * every screen quadrant maps to the same fold quadrant, so the picture can
//     only ever carry the FOLD's 4-fold symmetry, never the motif's.
//
// FOLDMODE (knob_166) selects the fold:
//   < 0.33  MIRROR   -- baseline triangle wave (byte-identical behaviour)
//   < 0.66  TILE     -- translation repeat, free twist    (whole motif, SEAMS)
//   >=0.66  LOCK     -- translation repeat, lattice-locked (whole motif, NO seams)
//
// TILE:  p = (fract(p + 0.5) - 0.5) * s
//   Same derivative magnitude (s) so `scale *= s` is unchanged; same cell pitch
//   (cells at integer p_in, one per unit period, 2 apart in post units) and the
//   same cell radius, so TILE vs MIRROR is apples-to-apples geometry. The output
//   now spans [-s/2, s/2) = [-1,1), so tc spans the WHOLE bake and the motif is
//   drawn once, whole, unmirrored, per cell, at every level.
//
// THE COST, and why it is a theorem and not a tuning bug:
//   A continuous periodic map onto a full tile MUST turn around inside the
//   period -- i.e. it must mirror. Unmirrored therefore REQUIRES a jump. Here p
//   jumps by exactly s at each tile wall. That jump is harmless to the level
//   that made it (the wall sits at |p| = s/2 = 1, outside the cell radius 0.6)
//   but it is fed through `p *= rot2(theta)` into every DEEPER level, where it
//   lands as an arbitrary phase shift -> visible seam lines.
//
// LOCK removes the seam exactly rather than hiding it. The deeper level applies
// fract() of period 1, so the jump is invisible to it iff the rotated jump
// vector is an INTEGER vector. |jump| = s, so that needs
//     s integer  AND  theta a multiple of PI/2.
// (cos,sin must both be in {0,+-1}; +-60 deg gives 2sin60 = 1.732, not integer.)
// So LOCK pins s = 2 and replaces the continuous per-level twist with a STATIC
// per-level quarter-turn count seeded per device. Free per-level TRANSLATIONS
// are still allowed -- an integer jump plus any constant is still an integer
// jump -- so variety is recovered as offsets, not as angle.
// What LOCK costs: gThetaStep (K141), gSpin, gTwistFall (K142), gLevelSkew (K145),
// evoWarp's fold term and the drifting fold ratio (K131) all stop acting on the
// fold. That is most of the kaleidoscope. Reported, not hidden.
#define FOLDMODE (knob_166)
...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        // ---- H12 FOLD SELECT (knob_166) ----
        float tileOn = step(0.33, FOLDMODE);   // translation repeat instead of mirror repeat
        float lockOn = step(0.66, FOLDMODE);   // + quantise s and theta so the tiling is seamless
        float s = mix(gScale, 2.0, lockOn);                   // FOLD RATIO drifts (2.0 ± 0.3, iter 12) — a fractal PERMUTATION; continuous across mirror cells for any s. LOCK pins it to an INTEGER: the tile-wall jump is |s|, and only an integer jump is invisible to the next level's period-1 fract().
        vec2 pMirror = 1.0 - abs(s * fract(p - 0.5) - s * 0.5);   // mirror-repeat fold (baseline): triangle wave, cell at the tile CORNER, motif mirrored 4x, only one quadrant of the bake reachable
        // LOCK recovers per-device / per-level variety as a free TRANSLATION of the tiling phase
        // (an integer jump plus any constant is still an integer jump). It goes INSIDE fract(), so
        // it moves where the cells are, never where the motif sits inside its cell.
        vec2 tileOff = vec2(sin(float(i) * 2.399 + seed3 * TAU), cos(float(i) * 1.771 + seed4 * TAU)) * 0.5 * lockOn;
        vec2 pTile   = (fract(p + 0.5 + tileOff) - 0.5) * s;      // H12 translation-repeat fold: same |derivative| (s) and same cell pitch, but the cell sits at the tile CENTRE and p spans [-s/2, s/2) so the WHOLE motif is drawn, once, unmirrored
        p = mix(pMirror, pTile, tileOn);
        ...
        // LOCK: replace the continuous twist with a STATIC per-level quarter turn. Anything other
        // than a multiple of PI/2 rotates the tile-wall jump off the integer lattice and the seam
        // comes back. Static (not drifting) so the lattice never SNAPS in time — geometry may
        // evolve, it may not jump. Per-device variety moves from angle to a free per-level OFFSET,
        // which costs nothing: an integer jump plus any constant is still an integer jump.
        float qStep  = 1.0 + floor(seed3 * 3.0);                         // 1..3 quarter turns per level
        float qTheta = (PI * 0.5) * floor(mod(float(i) * qStep + floor(seed4 * 4.0), 4.0));
        float qc = floor(cos(qTheta) + 0.5), qs = floor(sin(qTheta) + 0.5);   // EXACT 0/+-1
        p *= (lockOn > 0.5) ? mat2(qc, -qs, qs, qc) : rot2(theta);
        scale *= s;
        if (i < FIRST) continue;

        vec2 uv = abs(p);
        // H12: the MOTIF must be sampled UNMIRRORED. In MIRROR mode p is already >= 0 so this
        // mix is a no-op (which is exactly why deleting abs() was measured as a 0.36% no-op in
        // wave 1 — the fold had already done the mirroring). In TILE/LOCK mode p is signed and
        // `bp` is what lets a whole mon appear. The CROSS term below keeps `uv`: it is a
        // cell-centred plus sign and is 4-fold by construction, not by accident.
        vec2 bp = mix(uv, p, tileOn);
        ...
        float hexTerm  = hexDist(bp) - hexR_i;   // hexDist abs()es internally, so this is identical in every mode
        float beadTerm = beadDist(bp, hexR_i);
        ...
        // CROSS WEIGHT (knob_167, H12 preset dial). delt2 is the hex lattice's SKELETON: a filled
        // disc of radius gCross at the cell centre plus four arms out to the tile wall. It is
        // brighter than the motif and it is drawn ON TOP of the motif's middle, which is exactly
        // where a mon carries its identity. Under the mirror fold it read as part of the
        // kaleidoscope; under a translation fold it reads as a grid drawn over the beads. Pushing
        // it out of range hands the cell back to the motif. 0 = untouched, 1 = skeleton gone.
        // 0 = untouched, ~0.5 = skeleton CLIPPED to outside the motif (it becomes the string the
        //   beads are threaded on), 1 = skeleton gone entirely.
        delt2 = mix(delt2, max(delt2, hexR_i * 1.15 - length(uv)), step(0.25, knob_167));
        delt2 += step(0.75, knob_167) * 2.0;
