...
// ---- /lab INSTRUMENT (kiku.frag only — NOT for merge into 1.frag) ----
// onlyLevel: draw exactly ONE fold level, so "at which level does the motif stop being
// nameable" can be answered without the other five levels overlaying it. -1 = all (normal).
// flatLevels: bypass the depth window so a single level is not dimmed by its own depth.
uniform float onlyLevel;
uniform float flatLevels;
...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        scale *= s;
        if (i < FIRST) continue;
        if (onlyLevel >= 0.0 && abs(float(i) - onlyLevel) > 0.5) continue;   // /lab instrument
