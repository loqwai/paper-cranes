...
float noise(vec2 p) {
    ...
    float a = sin(i.x + i.y * VARIETY);
    float b = sin((i.x + 1.0) + i.y * VARIETY);
    float c = sin(i.x + (i.y + 1.0) * VARIETY);
    float d = sin((i.x + 1.0) + (i.y + 1.0) * VARIETY);
    d = mix(d, granular(random(p, 1.)+sin(time), 2.),0.5);
