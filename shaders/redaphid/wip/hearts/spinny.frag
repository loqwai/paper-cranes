...
#define HEART_COUNT 80.0    // Hearts per line
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for(float line = 0.0; line < LINE_COUNT; line++) {
        for(float i = 0.0; i < HEART_COUNT; i++) {
            ...
            if(d < 0.0) {
                // Modified color generation to prevent periodic issues
                vec3 col = 0.5 + 0.5 * cos(
                    vec3(0.0, 2.0, 4.0) +
                    line * 1.5 +
                    fract(t) * 8.0 + // Use fract(t) to prevent periodic overflow
                    mod(iTime, 10.0) + // Limit iTime range
                    PROBE_E * 4.0
                );
    ...
    // Modified background glow with better control
    float bgGlow = length(uv0);
    vec3 bgColor = vec3(0.15, 0.07, 0.2) * (1.0 - bgGlow) * (0.2 + PROBE_D * 0.3); // Reduced intensity

    finalColor = finalColor + bgColor;
    finalColor = clamp(finalColor, 0.0, 1.0);  // Clamp final output
