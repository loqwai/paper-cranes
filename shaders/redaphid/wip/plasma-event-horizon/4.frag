// @favorite: true
// @fullscreen: true
// @name: plasma event horizon 4
// @tags: black-hole, radial, event-horizon, oklch, monster-bass
//
// plasma-event-horizon/4.frag — forked from .../3.frag at iter 26 of /vibej run.
// Captures post-MONSTER_BASS-landing state: 16 wired knobs, slow-radiation
// aesthetic locked in, both feature-corner gates working (CALM_WARM, MONSTER_BASS),
// entropy-textured radiation, baseline floor, k6 RADIATION_RATE control. Snapshot
// of the deepest event-horizon territory yet — monster-bass amplitude boost
// landed, indomitable-monster aesthetic transferred to radial composition.
// See 4.md for the preset and full evolution history.
//
...
// Time advance — uses MEAN values (slow-changing baselines) for "slowish powerful" radiation
// rather than raw z-scores which jitter per-frame. Iter 19: user request for "slowish radiation
// that goes with the music" — replaced raw zScores with energyMean + half-strength roughness Z.
#define T_ADVANCE (energyNormalized * 0.6 + spectralRoughnessZScore * 0.5 + max(trebleZScore, 0.0) * 0.3)
...
// Monster-bass gate: fires when bass HIGH but mids/treble/energy LOW + dark centroid.
// This is the dr-fresch / sub-bass-only "lurking monster" pocket where bass-without-body
// dominates. Distinct from CALM_WARM (which needs mid-dominance). Iter 22 addition,
// thresholds widened iter 23 (mids was too strict — dr-fresch pockets often have mids 0.3-0.5).
#define MONSTER_BASS (smoothstep(0.5, 0.8, bassNormalized) * smoothstep(0.6, 0.2, midsNormalized) * smoothstep(0.4, 0.0, energyNormalized) * smoothstep(0.3, 0.0, spectralCentroidNormalized))
// #define MONSTER_BASS 0.0
...
// Singularity power — sustained POWERFUL pulse driven by slow audio means + bass-Z heartbeat.
// Iter 19 (user: "slowish powerful radiation"): re-balanced so the disk has a sustained baseline
// pump from energyMean (long-window average) rather than just transient z-spikes.
// energyMean is small (~0.003-0.01 in practice) so we use bassNormalized + midsNormalized as
// the "current loudness" signal. bassZ retained for kick punctuation but at lower coefficient.
#define HORIZON_POWER (1.0 + bassNormalized * 0.6 + midsNormalized * 0.4 + max(bassZScore, 0.0) * 0.8 + max(energyZScore, 0.0) * 0.4)
...
// DROP_FLARE_GAIN — knob_13: scales the drop punch intensity (auto-wired iter 15)
// User wiggled k13 +0.42 during a trebly+entropic+rising-energy buildup — wired to give them
// direct control over how hard the photon ring blooms on builds. Audio remains the trigger;
// k13 is the amplitude.
#define DROP_FLARE_GAIN (mix(0.0, 1.6, knob_13))
#define DROP_FLARE (smoothstep(0.3, 1.0, energyZScore) * 0.8 * DROP_FLARE_GAIN)
...
// BEAT_PUNCH — knob_15: scales a one-frame brightness snap on beat-detected frames
// Reduced 0.6 → 0.35 in iter 19 (user: "slowish radiation") — beats are now subtle taps,
// not strobing flashes. The slow radiation is the dominant rhythm now.
#define BEAT_PUNCH (knob_15 * (beat ? 1.0 : 0.0) * 0.35)

// POWERFUL_RADIATION — slow concentric outgoing waves driven by sustained bass + mids.
// Iter 19 (user: "powerful, slowish radiation that goes with the music"). Wave frequency is
// LOW (1.2 Hz visible, modulated to slower in calm passages), amplitude is HIGH on loud
// passages. Reads as gravitational waves rippling out from the singularity — the "slowish
// powerful" rhythm the user asked for. Phase always moves outward (negative iTime in arg).
// Loudness from bassNormalized + midsNormalized (sustained baselines, not z-score jitter).
// CRITICAL: phase argument must NOT depend on audio (causes per-frame phase jumps = strobing).
// Loudness GATES amplitude only. iter 19b fix.
#define RADIATION_LOUDNESS (bassNormalized * 0.7 + midsNormalized * 0.5)
// RADIATION_RATE — knob_6: how fast the slow concentric waves expand outward
// Auto-wired iter 20 (user's k6 wiggle + meditative tuning suggests they want a slowness lever)
// 0=very slow (~0.05Hz), 0.5=baseline (0.45Hz), 1=fast (1.0Hz). Phase only — never enter sin().
#define RADIATION_RATE (mix(0.05, 1.0, knob_6))
#define RADIATION_WAVES(r) (sin(r * 7.0 - iTime * RADIATION_RATE * 6.2831) * 0.5 + 0.5)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    // Outer rim: bright ring at the photon sphere
    // Treble-Z coefficient halved (was 0.4, now 0.2) iter 19 — was too twitchy
    // for the "slowish radiation" aesthetic.
    float photonRing = exp(-pow((r - (PHOTON_RING_RADIUS + RING_PULSE)) * 6.0, 2.0)) * (0.6 + max(bassZScore, 0.0) * 0.5 + max(trebleZScore, 0.0) * 0.2 + DROP_FLARE + BEAT_PUNCH);
    // POWERFUL RADIATION (iter 19): slow concentric outgoing waves modulating disk lightness.
    // Visible as bright→dim ripples expanding outward, breathing with the song's body.
    // Falls off near center (no waves through the singularity) and at edges (no waves in void).
    // Iter 21: amplitude scales with entropy. Iter 25: added 0.15 floor so the disk always
    // has a faint baseline radiation even in silent passages — preserves "always-radiating
    // presence" feel during dead air.
    // Iter 26: monster-bass pockets get extra radiation amplitude — gravitational waves
    // intensify in the lurking-monster regime. Subtle 1.3× max boost when MONSTER_BASS = 1.
    float radiation = RADIATION_WAVES(r) * smoothstep(0.05, 0.25, r) * smoothstep(0.85, 0.45, r) * (RADIATION_LOUDNESS + 0.15) * 0.32 * (1.0 + spectralEntropyNormalized * 0.4 + MONSTER_BASS * 0.3);
    ...
    // MONSTER_BASS pulls the void edge inward (compresses the bright zone) — deeper
    // darkness on sub-bass-only pockets. Iter 24: 0.1 → 0.18 for more visible compression.
    float voidFade = 1.0 - smoothstep(VOID_INNER - MONSTER_BASS * 0.18, VOID_OUTER - MONSTER_BASS * 0.18, r);
    ...
    // Iter 19: + radiation term gives sustained powerful slow ripples driven by sustained loudness.
    lch.x = clamp(lch.x * voidFade + coreGlow * 0.25 + photonRing * 0.22 + radiation + L_LIFT + BREATH * CALM_WARM * 0.06 - WARM_DEPTH + BRIGHT_LIFT * 0.35, 0.0, 1.0);
