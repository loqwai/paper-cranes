...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float zoomP = fract(bTime * max(0.0, 0.016 + EXA(knob_147, 0.030)) + flowPhase * 0.0045 * quietGate);   // K147 ZOOM RATE: how fast the perpetual self-similar zoom eats one octave (~60 s at centre). max() keeps it one-way.
