...
// LATTICE-VJ (3.frag — FORK of 2.frag at vj2 iter 17, 2026-08-18 exploration run) — a byte-copy of chromadepth-lattice/6.frag, kept as the SCRATCH COPY for
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    gHexR   = 0.60 + shapeB * 0.17 + midsLive * 0.05 * quietGate;   // iter 114: 0.06 → 0.17 — cell radius sweep must be VISIBLE (slow driver, safe)   // vj2 iter 2: 0.12·(1+wub·0.8) → 0.05 — the spring (settles ~0.4s) chases a 2–4 Hz wobble, so cells PULSED at wub rate = TWITCHY. Geometry follows slow music; the wub now lives in shading only.
    ...
    gCross  = 0.20 + shapeA * 0.11 - bassPulse * 0.02;   // iter 114: 0.04 → 0.11      // vj2 iter 2: .05 → .02 (de-twitch)
    ...
    gDepthFocus = clamp(0.30 + (waveletCentroidSpring - 0.5) * 0.8 + shapeA * 0.25 + (knob_132 - 0.5) * bank4, 0.0, 1.0);   // vj2 iter 12: 0.35/1.0 → 0.30/0.8 — on bright tracks the finest levels filled every cell with speckle (busy wallpaper); bias coarser, brightness pushes fine less hard   // biased COARSE (0.35): bold cells by default, filigree only when bright   // K132 DEPTH FOCUS   // BRIGHTNESS → fine detail, dark → coarse (level window)                            // fractal self-similarity ratio: slow permutation of the WHOLE structure (user iter 12: fractal permutations, not warps)
    ...
    //    + slow time. Intensity from treble spring + grit. Structural lighting, not colour flashing.
    {
        ...
        col *= 1.0 - shade * min(0.85, 0.22 + gKick * 0.95 + bassLive * 0.48)   /* iter 134: clamp relief stack — kick+wub together crushed lumMin to .063 (floor .08) */ * quietGate * mix(1.0, 0.4 + knob_136 * 1.2, bank4);   // K136 RELIEF DEPTH
