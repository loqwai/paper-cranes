...
vec4 fractal(vec2 p){
    ...
    for (int i = 0; i < LEVELS; i++){
        ...
        float delt1 = abs((hexDist(uv) - hexR_i) - gRingGap);   // K144 RING GAP: distance from the hex ring to the drawn line — sets how HOLLOW each cell is, independently of its radius.        // MIDS breathe the hexagons
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    gFill   = max(0.0, 0.06 + EXA(knob_149, 0.14)) + trebLive * 0.02 * quietGate + (spectralSpreadMedian - 0.26) * 0.05 * quietGate;   // iter150 HARMONIC WIDTH -> FILL: a wide, dense spectrum fills the cells solid; a narrow one hollows them to outlines.
    ...
    vec3 bg = lush(s + 0.5, 0.05) * max(0.0, 0.30 + EXB(knob_159, 0.55) + gComplex * 0.14);   // vj8-b8 RATCHET FLOOR COMPENSATION: b7's complexity ratchet draws finer generations as the set runs, and finer means THINNER LINES WITH MORE UNLIT GAP — measured dark fraction 0.37 -> 0.47 and lumMin down to 0.078 (floor is 0.08) over five minutes. The field floor now rises on the SAME monotonic set clock that causes it, so the gaps stay lit as the detail multiplies. Not the global multiplier and not audio-driven (directive #1 intact): this is the colour channel following the slowest clock there is.   // K159 BG FLOOR: how lit the empty field between cells is. Low = the lattice floats in the dark; high = no voids at all.                    // iter 27: DARK floor — the screenshot had no dark anywhere, so nothing could read                    // DARK muted field (3.frag) — the lattice + the sun are what you look at
