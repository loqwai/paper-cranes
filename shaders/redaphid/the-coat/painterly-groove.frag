...
// @mobile: true
// @tags: the-coat
// the-coat / painterly-groove: cool palette + heavy feedback + camera swagger MAX
// Source: the-coat-22 (Temptation - SIDEPIECE)
//
// URL: http://localhost:6969/jam.html?shader=redaphid/the-coat/painterly-groove&controller=the-coat&audio=tab
//
// Knobs:
...
#define RIM_BOOST (2.5 + spectralFluxNormalized * 2.0 + clamp(bassZScore, 0.0, 1.0) * 1.0 + smoothstep(0.6, 0.85, spectralEntropyNormalized) * smoothstep(0.5, 0.85, spectralRoughnessNormalized) * 1.5 + clamp(energyZScore, 0.0, 1.5) * 0.8 + pitch_change * 0.4)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // VJ STARFIELD — sparkle crosses in the upper sky, twinkle faster on treble
    {
        ...
        bg += vec3(1.0, 0.95, 0.85) * star * sky_mask * (1.3 + clamp(trebleZScore, 0.0, 2.5) * 0.8) * (1.0 - knob_10 * 0.5);
    }
