...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // BRACELET-NAVY: chrome locked to icy-white-blue rim. SEED_RIM_TILT shifts hue ±0.04
    // within the cool-blue family so each wearer's rim tone is unique but still navy-family.
    float chrome_hue = mix(0.58, 0.62, sin(time * 0.5) * 0.5 + 0.5) + SEED_RIM_TILT;
    ...
    // BRACELET-NAVY: lock coat to deep navy. Drop pulls toward slightly brighter navy, never cyan.
    ...
    // Lightness clamped LOW for navy darkness
    float fur_l_hi = 0.22 + bass_pulse * 0.10;
    float fur_l_lo = 0.14 + bass_pulse * 0.08;
