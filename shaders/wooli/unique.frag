...
#define J_REAL (-0.75 + sin(iTime * 0.04 * PHI) * 0.03)+((energyZScore)/50.)
// #define J_REAL 0.1
#define J_IMAG (0.186 + cos(iTime * 0.03 * SQRT2) * 0.025)

// Framing — seeds make each device unique via viewpoint, not fractal math
#define ZOOM_LVL (0.95 + seed3 * 0.1 + sin(iTime * 0.15 * PHI + seed3 * PI * 2.0) * 0.08 + energyMedian * 0.08 + bassNormalized * 0.12 + clamp(energyZScore, 0.0, 1.0) * 0.1)
...
#define MAMMOTH_SCALE (1.4 - bassNormalized * 0.15 - clamp(bassRSquared, 0.0, 1.0) * 0.25 - clamp(energyZScore, 0.0, 1.0) * 0.2)/1.2
