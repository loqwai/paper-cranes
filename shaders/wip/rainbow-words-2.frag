...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  for(float i = 1.0; i <= currentSteps; i += 0.5) {
    ...
    // Add plasma-like warping
    vec3 p = vec3(zoomedUV * 2.0 - 1.0, 0.0);
    float rz = map(p);
    float f = clamp((rz - map(p + 0.1)) * 0.5, -0.1, 1.0);

    // Apply warping to UV coordinates
    vec2 warpedUV = zoomedUV + vec2(
      sin(zoomedUV.y * 10.0 + iTime * 2.0 + rz) * 0.02,
      cos(zoomedUV.x * 10.0 + iTime * 2.0 + rz) * 0.02
    ) * f;
    ...
    vec3 ringColor = rainbowPalette(i / currentSteps + iTime * 0.1 + rz * 0.1);
    ...
    outlineColor -= ring * 2.0 * ringColor * falloff*2.;

    // Create enhanced flame effect using last frame
    vec2 flameUV = warpedUV;
    float warp = ring * 0.02 * (1.0 + f);
    flameUV += vec2(
      sin(zoomedUV.y * 10.0 + iTime * 2.0 + rz) * warp,
      cos(zoomedUV.x * 10.0 + iTime * 2.0 + rz) * warp
    );
    ...
    float flameIntensity = ring * falloff * (1.0 + f);
  ...
  // Add subtle color shifting based on plasma mapping
  vec3 p = vec3(uv * 2.0 - 1.0, 0.0);
  float rz = map(p);
  finalColor = mix(finalColor, rainbowPalette(rz * 0.1), 0.1);
