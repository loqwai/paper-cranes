...
vec3 palette( float t, float change ) {
    ...
    hsl.x = sin(pitchClassMean+hsl.x);
    col = hsl2rgb(hsl);
    return col;
...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    return length(max(d,0.0)) + min(max(d.x,d.y),0.0)*B;
}
...
float sdHexagram( in vec2 p, in float r )
{
    ...
    p = abs(p * Rot(T*0.1));
    //*****p *= Rot(T*0.1)// rotates something in here
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    //uv = fract(uv * 0.8) - 0.5;
    uv = uv + abs(8.0);
    uv *= Rot(T*0.1);//Rotates the whole canvas

for(float i = 0.0; i< 2.2; i++){ 
    vec2 fRot = fract(uv * Rot(T*0.1));
    vec2 uvRot = uv * Rot(T*0.1);
    vec2 uv0Rot = uv0 * Rot(T*0.1);
    ...
    //uv = (uv * 1.5) - 0.5 ;
    uv = abs((uv * 2.0)) - (0.8 *T/20000.0) ; //adding abs here undoes some transform to make it a fractal in the centyer again
    //uv = abs(fract(uv * 1.4) - 0.5) ; // addingt abs here makes it tile seamlessly 
    uv =  abs(uv * Rot(T*0.1));//rotates each individual uvm the multiplication iterates
    //uv = fract(uv * 0.8) - 0.5;
    //uv0 *= fRot;
    
    float d = length(uv) * exp(-length(uv0 *2.0 )); // THIS CHANGES THINGS BASED ON THE DISTANCE TO THE CENTER OF THE CANVAS         

    vec3 col = palette(length(uv) + i*5.6 + T*0.2, T);
    //col.x = col.x + T;

    
    d = sin(d *0.8 + T*0.1)/8.0;
    
    d += sdArc(uv,uv, 0.7, T*0.0002);
    ...
    d += sdfCoolS(uv0)*T*0.0002;
    
    //d += sdArc(uv,uv, 0.7, T*0.00002);
