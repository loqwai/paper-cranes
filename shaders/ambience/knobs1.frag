// @fullscreen: true
// https://visuals.beadfamous.com/edit?knob_2=0.26&knob_2.min=0&knob_2.max=1&knob_1=0.504&knob_1.min=0&knob_1.max=1&knob_3=0.52&knob_3.min=0&knob_3.max=1&knob_4=0.669&knob_4.min=0&knob_4.max=1&knob_5=1&knob_5.min=0&knob_5.max=1&knob_6=0.433&knob_6.min=0&knob_6.max=1&knob_7=0.402&knob_7.min=0&knob_7.max=1&knob_8=1&knob_8.min=0&knob_8.max=1&knob_9=0&knob_9.min=0&knob_9.max=1&knob_10=0.984&knob_10.min=0&knob_10.max=1&knob_11=0.252&knob_11.min=0&knob_11.max=1&knob_12=0&knob_12.min=0&knob_12.max=1&knob_16=0.669&knob_16.min=0&knob_16.max=1&knob_15=0.992&knob_15.min=0&knob_15.max=1&knob_14=0.433&knob_14.min=0&knob_14.max=1&knob_13=0.315&knob_13.min=0&knob_13.max=1
// --- Constants & Helpers ---
...
#define PHEROMONE_STRENGTH   (knob_17)
#define BLOB_THRESHOLD       (bass > 0.5 ? 0.5 : bass)
#define ENVIRONMENT_FOOD     (energy)
#define HUE_DAMPING_FACTOR   (0.75 + knob_18 * 0.20)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    lastCol.z = clamp(lastCol.z + (knob_4 - 0.5), 0.0, 1.0);
    lastCol.y = lastCol.y > 0.8
              ? lastCol.y
              : lastCol.y + (lastCol.z - 0.5);

    float t = time * (0.05 + 0.05 * knob_5);
    float baseScale  = 1.0 + knob_6 * 2.0;
    float extraScale = 1.0 + knob_7 * 2.0;
    p *= baseScale;

    p = domainWarp(p, knob_8 * length(prevRGB), t * 0.1);
    p = domainWarp(p, knob_9 * 0.3, -t * 0.07);

    float wave = animateEaseInOutSine(t * 0.1 + knob_10 * 0.3);
    p += wave * knob_10 * 0.3 * vec2(sin(p.y * 2.0), cos(p.x * 2.0));

    float base    = fbm(p * extraScale + t * 0.2);
    float f       = deepFractal(p * extraScale);
    float pattern = varied(p * mix(1.0, 4.0, knob_11) + t * 0.2, t);
    ...
    float hue      = fract(hueShift
                         ...
                         + (knob_2 * 0.2 + knob_2 * 0.3 + knob_5 * 0.25 + knob_8 * 0.2)
                         + lastCol.z);

    float sat = 0.8 + knob_11 * 0.2;
    ...
    newColor = mix(newColor, initTex, 0.05 * knob_15);
    ...
    finalHSL.x = fract(finalHSL.x + knob_19 * 0.05);
