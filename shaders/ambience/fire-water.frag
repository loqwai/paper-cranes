...
//https://visuals.beadfamous.com/edit?knob_18=0.97&knob_18.min=0&knob_18.max=1&knob_19=0.25&knob_19.min=0&knob_19.max=1&knob_21=0&knob_21.min=0&knob_21.max=1&knob_22=0.92&knob_22.min=0&knob_22.max=1&knob_17=0&knob_17.min=0&knob_17.max=1&knob_20=0.87&knob_20.min=0&knob_20.max=1&knob_23=1&knob_23.min=0&knob_23.max=1&knob_25=0.76&knob_25.min=0&knob_25.max=1&knob_24=1&knob_24.min=0&knob_24.max=1
...
float deepFractal(vec2 p){
    ...
    for(int i=0;i<4;i++){
        vec2 q = domainWarp(p, knob_8*0.35*amp, time*0.03*float(i));
        sum   += amp * fbm(q);
        p     *= 2.0;
        amp   *= 0.55;
    }
...
float varied(vec2 p,float t){
    ...
    float m = knob_15;                        // morph
    return mix( mix(a,b,m), c, knob_17 );     // 2‑stage blend
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    lastCol.z = clamp(lastCol.z + (knob_25-0.5), 0., 1.);
    lastCol.y = lastCol.y > 0.8 ? lastCol.y : lastCol.y + (lastCol.z - 0.5);
    // Time factor
    float t = time * (0.05 + 0.05 * knob_18); // knob_7 -> overall speed

    // Combine scale from knob_9 and knob_10
    float baseScale = 1.0 + knob_9 * 2.0; // knob_9 -> base scale
    float extraScale = 1.0 + knob_10 * 2.0; // knob_10 -> additional scale
    p *= baseScale;

    // Warp pass #1
    p = domainWarp(p, knob_19 * length(prevRGB), t * 0.1);  // knob_8 -> warp strength

    // Warp pass #2, let knob_1 intensify
    p = domainWarp(p, knob_7 * 0.3, -t * 0.07);
...
// ─── inside mainImage(), replace pattern calc ───────────────
    float pattern = varied(p*mix(1.0,4.0,knob_13)+t*0.2, t);
    pattern = mix(pattern, pattern*pattern, knob_16); // contrast warp

    // Hue shift: slow drift plus knob_11
    float hueShift = t * (0.1 + knob_11 * 0.2);
    ...
    float sat = 0.8 + knob_13 * 0.2; // knob_13 -> saturation
    ...
    lit += knob_2 * 0.1;
    // Use knob_12 as overall brightness multiplier
    lit *= (1.0 + knob_12 * 1.0);
    ...
    #define FEEDING_EFFICIENCY (knob_22) // How much saturation increases upon feeding
    ...
    #define PHEROMONE_STRENGTH (knob_21)   // Attraction/Repulsion based on avg neighbor hue
    #define BLOB_THRESHOLD (knob_22 * 0.5)    // Luminance threshold below which feeding/strong grouping occurs
    #define ENVIRONMENT_FOOD (knob_17 * 0.1)  // Ambient energy available
    #define HUE_DAMPING (0.75 + knob_23 * 0.20) // Constrained damping factor [0.75, 0.95]
    ...
    // Feedback blending with knob_14 (using the life-adjusted color)
    // Lower knob_14 -> stronger feedback from last frame
    ...
    finalHSL.x = fract(finalHSL.x + knob_17 * 0.05);
