...
float map(vec3 p) {
  p.xz *= m(LOCAL_TIME * 0.4);
  p.xy *= m(LOCAL_TIME * 0.3);
  vec3 q = p * 2.0 + LOCAL_TIME;
  return length(p + vec3(sin(LOCAL_TIME * 0.7))) * log(length(p) + 1.0) +
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 baseColor = mix(getInitialFrameColor(uv), getLastFrameColor(uv), sin(LOCAL_TIME+EPSILON)/2.*getLastFrameColor(uv*1.1).x+EPSILON).rgb;
  ...
  float radiationPhase = radiationFactor * 6.2831 + LOCAL_TIME * 2.0;

  // Smooth zoom animation with plasma influence
  float zoomPhase = animateEaseInOutCubic(sin(LOCAL_TIME * 0.5) * 0.5 + 0.5);
  float maxSteps = 12.0;
  float stepVariation = sin(LOCAL_TIME * 0.5) * 4.0;
  float currentSteps = maxSteps + stepVariation;
  float zoomStep = -0.1 - (sin(LOCAL_TIME * 0.5) * 0.005);
  ...
  for(float i = 1.0; i <= currentSteps; i += 0.5) {
    ...
    vec3 ringColor = rainbowPalette(i / currentSteps + LOCAL_TIME * 0.1 + rz * 0.1 + radiationPhase * 0.1);
    ...
    flameColor += flameRingColor * flameIntensity * (1.0 + sin(LOCAL_TIME * 3.0 + i + rz) * 0.3);
  }
