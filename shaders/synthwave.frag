...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //if (abs(uv.x) < (9.0 / 16.0))
    {
        ...
        if (uv.y < -0.2)
        {
            ...
            col = mix(col, vec3(spectralCrestNormalized, spectralRoughnessNormalized, 1.0), gridVal);
        }
        else
        {
            float fujiD = min(uv.y * energy - 0.5, 1.0);
            ...
            // fuji outline
            col = mix( col, vec3(1.0, 0.5, spectralSkew/24.), 1.0-smoothstep(0.0,0.01,abs(fujiVal)) );
        ...
        // rotate uv by time
        uv = vec2(uv.x * cos(iTime * 0.1) - uv.y * sin(iTime * 0.1), uv.x * sin(iTime * 0.1) + uv.y * cos(iTime * 0.1));
        // get last frame color
        vec4 lastCol = getLastFrameColor(uv);
        fragColor = mix(vec4(col, 1.0),lastCol, clamp(0.2, .8, energyZScore));
    }
