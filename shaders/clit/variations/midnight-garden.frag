...
// Midnight Garden — Soft y-dominant petals, gentle drift, medium trails

#define A (1.625 + spectralCentroidSlope * spectralCentroidRSquared * 0.28 + spectralCentroidZScore * 0.0525 + 0.018 * sin(iTime * 0.033))
#define B (0.52 + energySlope * energyRSquared * 0.25 + energyZScore * 0.03 + 0.02 * sin(iTime * 0.036))
...
#define FEEDBACK_MIX (0.35 + energyNormalized * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 1.6 + midsNormalized * 3.0;
    ...
    if (beat) { col += vec3(0.04, 0.12, 0.04) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.9, 0.88, 0.9));
