...
// Deep Sea — Smooth wide forms, glacial drift, heavy ghosted trails

#define A (1.4 + spectralCentroidSlope * spectralCentroidRSquared * 0.32 + spectralCentroidZScore * 0.06 + 0.015 * sin(iTime * 0.03))
#define B (0.58 + energySlope * energyRSquared * 0.25 + energyZScore * 0.03 + 0.02 * sin(iTime * 0.036))
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 1.2 + spectralEntropyNormalized * 2.8;
    ...
    if (beat) { col += vec3(0.02, 0.08, 0.12) * focal; col *= 1.05; }
    col *= PULSE;
