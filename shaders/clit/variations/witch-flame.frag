...
// Witch Flame — Upward flame drift, y-dominant tendrils, roughness-swept color

#define A (1.65 + spectralCentroidSlope * spectralCentroidRSquared * 0.32 + spectralCentroidZScore * 0.06 + 0.022 * sin(iTime * 0.037))
#define B (0.52 + energySlope * energyRSquared * 0.3 + energyZScore * 0.036 + 0.025 * sin(iTime * 0.0405))
...
#define FLOW_Y (spectralSpreadSlope * 0.002 + -0.0008)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 5.0 + spectralRoughnessNormalized * 3.2;
    ...
    if (beat) { col += vec3(0.04, 0.12, 0.05) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.9, 0.89, 0.9));
