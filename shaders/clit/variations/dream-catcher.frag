...
// Dream Catcher — Tight mesh web, high complexity, pastel rainbow cycling

#define A (1.85 + spectralCentroidSlope * spectralCentroidRSquared * 0.4 + spectralCentroidZScore * 0.075 + 0.022 * sin(iTime * 0.037))
#define B (0.45 + energySlope * energyRSquared * 0.25 + energyZScore * 0.03 + 0.028 * sin(iTime * 0.0432))
...
#define FEEDBACK_MIX (0.28 + energyNormalized * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float lace_x = smoothstep(-1.0, -4.5, log(max(x, 1e-10)));
    float lace_y = smoothstep(-1.0, -4.5, log(max(y, 1e-10)));
    ...
    float color_phase = -0.3 + spectralCentroidNormalized * 3.5;
    ...
    if (beat) { col += vec3(0.1, 0.04, 0.08) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.89, 0.9, 0.88));
