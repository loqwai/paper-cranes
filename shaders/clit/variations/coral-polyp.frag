...
// Coral Polyp — Twitchy tendrils, x-dominant wing spread, fast detail cycling

#define A (1.75 + spectralCentroidSlope * spectralCentroidRSquared * 0.4 + spectralCentroidZScore * 0.075 + 0.02 * sin(iTime * 0.035))
#define B (0.48 + energySlope * energyRSquared * 0.3 + energyZScore * 0.036 + 0.025 * sin(iTime * 0.0405))
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float lace = max(lace_x, lace_y * 0.35);
    float lace_fine = lace_x * lace_y;
    lace = pow(max(lace, 0.0), 3.5);
    // Rainbow with audio-driven color phase
    float color_phase = 0.4 + pitchClassNormalized * 3.2;
    ...
    vec3 bg = vec3(0.04, 0.01, 0.03);
    vec3 col = mix(bg, sexy_col, lace);
    col += vec3(0.65, 0.6, 0.7) * lace_fine * 0.25;
    ...
    vec3 gem_base = vec3(1.0, 0.45, 0.35);
    ...
    if (beat) { col += vec3(0.12, 0.04, 0.02) * focal; col *= 1.05; }
    col *= PULSE;
