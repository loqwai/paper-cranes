...
// Breathing Tides — Slow zoom breathing, high B smoothness, heavy persistent trails

#define A (1.55 + spectralCentroidSlope * spectralCentroidRSquared * 0.24 + spectralCentroidZScore * 0.045 + 0.016 * sin(iTime * 0.031))
#define B (0.6 + energySlope * energyRSquared * 0.2 + energyZScore * 0.024 + 0.02 * sin(iTime * 0.036))
...
#define FEEDBACK_MIX (0.48 + energyNormalized * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 2.0 + bassNormalized * 3.0;
    ...
    if (beat) { col += vec3(0.04, 0.06, 0.1) * focal; col *= 1.05; }
    col *= PULSE;
