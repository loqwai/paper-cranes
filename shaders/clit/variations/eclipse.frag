...
// Eclipse — Grand smooth corona, glacial sweep, strong rim glow

#define A (1.3 + spectralCentroidSlope * spectralCentroidRSquared * 0.32 + spectralCentroidZScore * 0.06 + 0.012 * sin(iTime * 0.027))
#define B (0.62 + energySlope * energyRSquared * 0.2 + energyZScore * 0.024 + 0.018 * sin(iTime * 0.0342))
...
#define FEEDBACK_MIX (0.2 + energyNormalized * 0.1)
#define RIM_INTENSITY (0.5 + trebleNormalized * 0.7)
#define GEM_BRILLIANCE (0.8 + spectralCrestNormalized * 0.5)
#define GEM_DISPERSION (0.3 + spectralSpreadNormalized * 0.4)
#define FLAP_RATE 0.04
#define FLAP_AMP 0.6
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 2.0 + spectralRoughnessNormalized * 2.5;
    ...
    if (beat) { col += vec3(0.12, 0.08, 0.02) * focal; col *= 1.05; }
    col *= PULSE;
