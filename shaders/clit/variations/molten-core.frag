...
// Molten Core — Wide A morph range, dramatic shape-shifting, medium trails

#define A (1.6 + spectralCentroidSlope * spectralCentroidRSquared * 0.48 + spectralCentroidZScore * 0.09 + 0.025 * sin(iTime * 0.04))
#define B (0.53 + energySlope * energyRSquared * 0.375 + energyZScore * 0.045 + 0.03 * sin(iTime * 0.045))
...
#define FEEDBACK_MIX (0.32 + energyNormalized * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = -0.8 + energyNormalized * 3.5;
    ...
    if (beat) { col += vec3(0.15, 0.06, 0.02) * focal; col *= 1.05; }
    col *= PULSE;
