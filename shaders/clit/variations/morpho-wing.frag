...
// Morpho Wing — Fast flap, x-wing spread, time-rotating color + audio sweep

#define A (1.675 + spectralCentroidSlope * spectralCentroidRSquared * 0.28 + spectralCentroidZScore * 0.0525 + 0.022 * sin(iTime * 0.037))
#define B (0.5 + energySlope * energyRSquared * 0.3 + energyZScore * 0.036 + 0.025 * sin(iTime * 0.0405))
...
#define FEEDBACK_MIX (0.22 + energyNormalized * 0.1)
...
#define FLAP_RATE 0.4
#define FLAP_AMP 0.55
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 0.0 + spectralCentroidNormalized * 3.5 + iTime * 0.04;
    ...
    if (beat) { col += vec3(0.06, 0.06, 0.12) * focal; col *= 1.05; }
    col *= PULSE;
