...
// Nebula Birth — Slow wide drift, heavy trails, time-rotating color spiral

#define A (1.475 + spectralCentroidSlope * spectralCentroidRSquared * 0.36 + spectralCentroidZScore * 0.0675 + 0.014 * sin(iTime * 0.029))
#define B (0.57 + energySlope * energyRSquared * 0.25 + energyZScore * 0.03 + 0.018 * sin(iTime * 0.0342))
...
#define FLAP_RATE 0.07
#define FLAP_AMP 0.45
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 2.5 + spectralSkewNormalized * 3.2 + iTime * 0.03;
    ...
    if (beat) { col += vec3(0.08, 0.04, 0.12) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.9, 0.91, 0.89));
