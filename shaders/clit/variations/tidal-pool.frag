...
// Tidal Pool — Lateral drift, balanced detail, crest-driven color cycling

#define A (1.6 + spectralCentroidSlope * spectralCentroidRSquared * 0.32 + spectralCentroidZScore * 0.06 + 0.02 * sin(iTime * 0.035))
#define B (0.54 + energySlope * energyRSquared * 0.3 + energyZScore * 0.036 + 0.025 * sin(iTime * 0.0405))
...
#define FEEDBACK_MIX (0.33 + energyNormalized * 0.1)
...
#define FLAP_RATE 0.12
#define FLAP_AMP 0.35
...
#define FLOW_X (spectralCentroidSlope * 0.003 + 0.0004)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    lace = pow(max(lace, 0.0), 2.8);
    // Rainbow with audio-driven color phase
    float color_phase = 3.5 + spectralCrestNormalized * 2.8;
    ...
    if (beat) { col += vec3(0.04, 0.08, 0.1) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.9, 0.89, 0.89));
