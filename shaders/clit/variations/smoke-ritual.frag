...
// Smoke Ritual — Upward drift, heavy trails, time-rotating color + entropy sweep

#define A (1.5 + spectralCentroidSlope * spectralCentroidRSquared * 0.32 + spectralCentroidZScore * 0.06 + 0.015 * sin(iTime * 0.03))
#define B (0.54 + energySlope * energyRSquared * 0.25 + energyZScore * 0.03 + 0.02 * sin(iTime * 0.036))
...
#define FEEDBACK_MIX (0.45 + energyNormalized * 0.1)
...
#define FLOW_Y (spectralSpreadSlope * 0.002 + -0.0006)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 0.3 + spectralEntropyNormalized * 3.0 + iTime * 0.02;
    ...
    if (beat) { col += vec3(0.1, 0.05, 0.04) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.9, 0.91, 0.92));
