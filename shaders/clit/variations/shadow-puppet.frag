...
// Shadow Puppet — Inverted: dark lace silhouette on warm lit background

#define A (1.575 + spectralCentroidSlope * spectralCentroidRSquared * 0.28 + spectralCentroidZScore * 0.0525 + 0.02 * sin(iTime * 0.035))
#define B (0.55 + energySlope * energyRSquared * 0.25 + energyZScore * 0.03 + 0.022 * sin(iTime * 0.0378))
...
#define FEEDBACK_MIX (0.15 + energyNormalized * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    lace = pow(max(lace, 0.0), 4.0);
    // Rainbow with audio-driven color phase
    float color_phase = 0.6 + trebleNormalized * 2.8;
    ...
    vec3 bg_glow = vec3(0.06, 0.04, 0.02) + sexy_col * 0.15;
    ...
    if (beat) { col += vec3(0.1, 0.06, 0.02) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.89, 0.9, 0.91));
