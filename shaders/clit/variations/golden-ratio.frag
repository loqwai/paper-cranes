...
// Golden Ratio — Phi-based multi-sine curl, balanced detail, slow organic drift

#define A (1.7 + spectralCentroidSlope * spectralCentroidRSquared * 0.24 + spectralCentroidZScore * 0.045 + 0.02 * sin(iTime * 0.035))
#define B (0.5 + energySlope * energyRSquared * 0.3 + energyZScore * 0.036 + 0.022 * sin(iTime * 0.0378))
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 0.8 + spectralKurtosisNormalized * 3.0;
    ...
    vec3 gem_base = vec3(1.0, 0.8, 0.35);
    ...
    if (beat) { col += vec3(0.12, 0.06, 0.02) * focal; col *= 1.05; }
    col *= PULSE;
