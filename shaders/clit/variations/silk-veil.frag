...
// Silk Veil — Soft low-power lace, heavy feedback ghosting, gentle motion

#define A (1.5 + spectralCentroidSlope * spectralCentroidRSquared * 0.24 + spectralCentroidZScore * 0.045 + 0.016 * sin(iTime * 0.031))
#define B (0.56 + energySlope * energyRSquared * 0.2 + energyZScore * 0.024 + 0.02 * sin(iTime * 0.036))
...
#define FEEDBACK_MIX (0.5 + energyNormalized * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    lace = pow(max(lace, 0.0), 1.8);
    // Rainbow with audio-driven color phase
    float color_phase = -0.2 + spectralRolloffNormalized * 2.8;
    ...
    if (beat) { col += vec3(0.1, 0.04, 0.06) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.9, 0.91, 0.9));
