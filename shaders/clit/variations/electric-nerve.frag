...
// Electric Nerve — Dense fiber detail, fast twitchy x-spread, sharp edges

#define A (1.95 + spectralCentroidSlope * spectralCentroidRSquared * 0.4 + spectralCentroidZScore * 0.075 + 0.03 * sin(iTime * 0.015))
#define B (0.42 + energySlope * energyRSquared * 0.35 + energyZScore * 0.042 + 0.035 * sin(iTime * 0.0195))
...
#define FEEDBACK_MIX (0.12 + energyNormalized * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = 3.2 + spectralFluxNormalized * 3.8;
    ...
    if (beat) { col += vec3(0.03, 0.1, 0.12) * focal; col *= 1.05; }
    col *= PULSE;
