...
// Heartbeat — Bass-driven pulse, breathing zoom, strong beat flash

#define A (1.5 + spectralCentroidSlope * spectralCentroidRSquared * 0.24 + spectralCentroidZScore * 0.045 + 0.02 * sin(iTime * 0.035))
#define B (0.55 + energySlope * energyRSquared * 0.3 + energyZScore * 0.036 + 0.025 * sin(iTime * 0.0405))
...
#define PULSE (1.0 + bassZScore * 0.1)
...
void mainImage(out vec4 P, vec2 V) {
    ...
    float color_phase = -0.5 + bassNormalized * 3.2;
    ...
    if (beat) { col += vec3(0.15, 0.03, 0.02) * focal; col *= 1.05; }
    col *= PULSE;
    ...
    col = pow(max(col, vec3(0.0)), vec3(0.89, 0.91, 0.9));
