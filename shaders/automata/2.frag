...
#define MUSIC_THRESHOLD (beat ? 0.02: 0.01)
...
vec4 play(vec2 uv) {
    // rotate uv over time
    uv -= 0.5;
    uv *= mat2(cos(iTime/100.), -sin(iTime/100.), sin(iTime/100.), cos(iTime/100.));
    uv += 0.5;
    ...
    if (distance(uv, aliveUv1) < MUSIC_THRESHOLD || distance(uv, aliveUv2) < MUSIC_THRESHOLD || distance(uv, aliveUv3) < MUSIC_THRESHOLD){
    ...
    if (isAlive(last)) {
        ...
        else if (aliveCount > (beat ? 7 : 4)) return last * vec4(0.7647, 0.0431, 0.8157, 1.0);
