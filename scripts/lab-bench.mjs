...
in vec2 a; void main(){ gl_Position = vec4(a,0.,1.); }`)
    ...
    const res = {}
    for (const a of arms) {
        const [label, k161, k162] = a.split(':')
        const samples = []
        for (let r = 0; r < 5; r++) samples.push(run(+k161, +k162, 30))
        samples.sort((x, y) => x - y)
        res[label] = { medianMsPerFrame: +samples[2].toFixed(3), all: samples.map(s => +s.toFixed(3)) }
    }
