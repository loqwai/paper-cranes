...
const PORT = 6983
const BASE = `http://localhost:${PORT}/?shader=${process.env.SHADER || "redaphid/wip/lattice-bead/split"}&controller=lattice-nav&wavelet=true&noaudio=true&fullscreen=true&navZoom=0.218&image=images/beads/mon-kiku.png&knob_1=0.429&knob_134=0.507&knob_144=0.3`
...
for (const pair of pairs) {
    ...
    const guard = await page.evaluate(() => {
        ...
        return { port: location.port, shader: p.get('shader'), k161: p.get('knob_161'),
                 k162: p.get('knob_162'), t: p.get('time'), canvas: c.width + 'x' + c.height,
